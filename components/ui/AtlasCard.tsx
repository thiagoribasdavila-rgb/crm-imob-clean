...
export function AtlasCardHeader({ eyebrow, title, description, action }: { eyebrow?: string; title: string; description?: string; action?: ReactNode }) {
  return (
...
export function AtlasMetric({ label, value, detail, trend, tone = "blue" }: { label: string; value: ReactNode; detail?: string; trend?: string; tone?: "blue" | "green" | "amber" | "violet" | "rose" }) {
  return (
    <article className={`atlas-metric atlas-metric-${tone}`}>
      <div className="flex items-start justify-between gap-3">
        <p className="text-sm text-slate-400">{label}</p>
        {trend ? <span className="rounded-full border border-white/10 bg-white/[0.05] px-2 py-1 text-[10px] font-semibold text-slate-300">{trend}</span> : null}
      </div>
