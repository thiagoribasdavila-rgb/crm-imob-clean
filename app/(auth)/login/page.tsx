      ...
      <div className="relative mx-auto grid min-h-screen w-full max-w-[1440px] items-center gap-12 px-5 py-8 lg:grid-cols-[1.1fr_440px] lg:px-12 xl:gap-24">
        <section className="relative hidden min-h-[720px] overflow-hidden border-r border-white/[.06] p-10 lg:flex lg:flex-col lg:justify-between xl:p-12">
          ...
          <Link href="/" className="inline-flex items-center gap-4" aria-label="Voltar para a página inicial do Atlas">
            ...
            <div><p className="text-2xl font-black tracking-[-.04em]">ATLAS <span className="text-sky-400">ONE</span></p><p className="text-[10px] font-semibold uppercase tracking-[.25em] text-slate-500">Real Estate Intelligence</p></div>
          </Link>
          <div className="mt-12 inline-flex items-center gap-2 rounded-full border border-sky-300/10 bg-sky-300/[.045] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.18em] text-sky-200"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-sky-300" /> Inteligência conectada ao comercial</div>
        ...
        <section className="mx-auto w-full max-w-lg">
          <div className="mb-6 flex items-center justify-between gap-3 lg:hidden"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-sky-400/15 font-black text-sky-300">A</span><div><p className="text-xl font-black">ATLAS <span className="text-sky-400">ONE</span></p><p className="text-[9px] uppercase tracking-[.2em] text-slate-500">Real Estate Intelligence</p></div></div><span role="img" aria-label="Assistente Atlas" className="h-16 w-12 bg-[url('/brand/atlas-robot-assistant.png')] bg-contain bg-center bg-no-repeat drop-shadow-[0_12px_20px_rgba(14,165,233,.18)]" /></div>
          <div className="relative overflow-hidden rounded-[32px] border border-white/[.09] bg-[#070d15]/92 p-6 shadow-[0_40px_120px_rgba(0,0,0,.55),inset_0_1px_0_rgba(255,255,255,.035)] backdrop-blur-2xl sm:p-9">
            ...
            <form className="mt-8 space-y-5" onSubmit={handleSubmit} noValidate>
              ...
              <button type="submit" disabled={loading} aria-busy={loading} className="group flex h-14 w-full items-center justify-center gap-2 rounded-full bg-white px-5 text-sm font-bold !text-slate-950 shadow-[0_16px_45px_rgba(255,255,255,.08)] transition hover:-translate-y-0.5 hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-60">
                {loading ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" aria-hidden="true" /> {loginStage === "profile" ? "Preparando sua operação..." : loginStage === "redirect" ? "Abrindo seu painel..." : "Validando acesso..."}</> : <>Entrar no Atlas One <span aria-hidden="true">→</span></>}
              </button>
            ...
            <div className="mt-7 border-t border-white/[0.07] pt-5">
              <div className="flex flex-wrap items-center justify-between gap-3 text-[10px] font-semibold uppercase tracking-[.12em] text-slate-600"><span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-sky-400" /> Sessão protegida</span><span>Dados isolados</span><span>Atlas One</span></div>
            </div>
          </div>
