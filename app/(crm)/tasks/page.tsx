...
export default function TasksPage() {
  ...
  const [kanbanContextApplied, setKanbanContextApplied] = useState(false);

  const kanbanTaskContext = useMemo(() => {
    if (
      searchParams.get("from") !== "pipeline" ||
      searchParams.get("intent") !== "task"
    ) {
      return null;
    }

    const action = searchParams.get("action") || "Executar próximo contato";
    const lead = searchParams.get("lead") || "Lead";
    const project = searchParams.get("project") || "";
    const score = Number(searchParams.get("score") || 0);
    const stage = searchParams.get("stage") || "novo";
    const temperature = searchParams.get("temperature") || "frio";

    return {
      description: [
        "Criada a partir do Kanban.",
        `Lead: ${lead}`,
        `Etapa: ${stage}`,
        ...
        .filter(Boolean)
        .join("\n"),
      dueAt: toDatetimeLocal(searchParams.get("suggestedAt")),
      leadId: searchParams.get("leadId") || "",
      priority: score >= 70 || temperature === "quente" ? "alta" : "media",
      title: action,
    };
  }, [searchParams]);
  ...
  useEffect(() => {
    if (!kanbanTaskContext || kanbanContextApplied) return;

    setShowCreate(true);
    setView("priority");
    setForm((current) => ({
      ...current,
      description: current.description || kanbanTaskContext.description,
      dueAt: current.dueAt || kanbanTaskContext.dueAt,
      leadId: current.leadId || kanbanTaskContext.leadId,
      priority:
        current.priority === EMPTY_FORM.priority
          ? kanbanTaskContext.priority
          : current.priority,
      title: current.title || kanbanTaskContext.title,
    }));
    setKanbanContextApplied(true);
  }, [kanbanContextApplied, kanbanTaskContext]);
  ...
  return (
    <div
      ...
      data-phase="38-task-execution-workspace 111-kanban-context-intake"
      data-ux-phase="22-tasks-agenda-focus"
      data-task-layout="execution-first"
    >
      <section className="atlas-task-hero" aria-labelledby="atlas-task-title" data-page-header="decision" data-primary-action-count="1">
        <div className="atlas-task-hero-copy">
          <AtlasBadge tone="violet">ROTINA COMERCIAL</AtlasBadge>
      ...
      <KanbanHandoffBanner module="tasks" />
      ...
      {leadership ? (
        <AtlasDetailDisclosure label="Ver carga da equipe por responsável">
          <AtlasCard className="h-fit">
              ...
              <p className="border-t border-white/[.06] px-5 py-3 text-[10px] leading-4 text-slate-600">
