  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <Link
        ...
        className={`inline-flex w-fit items-center gap-2 text-sm text-[var(--atlas-texto-medio)] underline underline-offset-2 hover:text-[var(--atlas-texto-forte)] ${focusRing}`}
      >
      ...
      {notice ? (
        <div className="cc6-sev-band cc6-panel-quiet py-3 pl-4 pr-3 text-sm leading-6 text-[var(--atlas-estado-sucesso)]" role="status" style={{ "--cc6-sev": "#34d399" } as CSSProperties}>{notice}</div>
      ) : null}
      ...
      <section className="cc6-panel-quiet cc6-reveal p-4" style={{ animationDelay: "60ms" }} aria-label="Consentimento e supressões">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1.5 max-w-3xl text-xs leading-5 text-[var(--atlas-texto-medio)]">Verificação na importação e novamente antes da aprovação: opt-outs e telefones inválidos permanecem bloqueados nas próximas importações, e nenhuma lead existente é transferida silenciosamente.</p>
          </div>
      ...
      {data?.experiences.length ? (
        <section className="cc6-panel cc6-reveal p-4 sm:p-5" style={{ animationDelay: "120ms" }} aria-labelledby="experiences-title">
          <header>
            ...
            <h2 id="experiences-title" className="mt-1 text-lg font-semibold tracking-tight text-[var(--atlas-texto-forte)]">Atendimentos que pedem decisão</h2>
            <p className="mt-1 text-xs leading-5 text-[var(--atlas-texto-fraco)]">A IA explica o atrito, mas nunca troca o corretor — a decisão e o motivo ficam auditados.</p>
          </header>
      ...
      <section className="cc6-panel cc6-reveal p-4 sm:p-5" style={{ animationDelay: "180ms" }} aria-labelledby="offers-title">
        <header className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 id="offers-title" className="mt-1 text-lg font-semibold tracking-tight text-[var(--atlas-texto-forte)]">Fila segura das bases antigas</h2>
            <p className="mt-1 text-xs leading-5 text-[var(--atlas-texto-fraco)]">A IA prioriza e sugere a próxima abordagem; o corretor decide e executa.</p>
          </div>
              ...
              return (
                <article key={offer.contactId} className="grid gap-3 border-t border-[rgba(148,163,184,0.12)] py-4 first:border-t-0 lg:grid-cols-[1fr_1fr_auto] lg:items-center">
                  <div className="min-w-0">
                    ...
                    <a href={`/leads/${offer.lead.id}`} className={`mt-2 block rounded-md text-sm font-semibold text-[var(--atlas-texto-forte)] transition-colors hover:text-[color:var(--atlas-accent-hover)] ${focusRing}`}>{offer.lead.name}</a>
                    <p className="mt-0.5 text-xs leading-5 text-[var(--atlas-texto-fraco)]">{offer.aiSuggestion}</p>
                  </div>
      ...
      <section id="nova-base" className="grid items-start gap-4 xl:grid-cols-[1.15fr_.85fr]">
        ...
        <section className="cc6-panel cc6-reveal p-4 sm:p-5" style={{ animationDelay: "240ms" }} aria-labelledby="import-form-title">
          <header>
            ...
            <h2 id="import-form-title" className="mt-1 text-lg font-semibold tracking-tight text-[var(--atlas-texto-forte)]">{isBroker ? "Minha base externa" : "Nova base para ativação"}</h2>
            <p className="mt-1 text-xs leading-5 text-[var(--atlas-texto-fraco)]">Formato: nome; telefone; e-mail. Até 500 contatos por base.</p>
          </header>
          <div className="mt-4 space-y-3">
            ...
            <label className="block cursor-pointer rounded-xl border border-dashed border-[rgba(148,163,184,0.3)] p-4 text-center text-sm text-[var(--atlas-texto-medio)] transition-colors focus-within:border-[color:var(--atlas-accent)] hover:border-[color:var(--atlas-accent)] hover:text-[var(--atlas-texto-forte)]">
            ...
            <div className="cc6-panel-quiet p-4">
              <p className="text-sm font-semibold text-[var(--atlas-texto-forte)]"><span className="cc6-num">{rows.length}</span> contatos reconhecidos</p>
        ...
        <section className="cc6-panel cc6-reveal p-4 sm:p-5" style={{ animationDelay: "300ms" }} aria-labelledby="activation-form-title">
          <header>
            ...
            <h2 id="activation-form-title" className="mt-1 text-lg font-semibold tracking-tight text-[var(--atlas-texto-forte)]">Preparar campanha de reativação</h2>
            <p className="mt-1 text-xs leading-5 text-[var(--atlas-texto-fraco)]">Use o nome técnico de um template já aprovado no Meta WhatsApp Manager.</p>
          </header>
          <div className="mt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block text-xs text-[var(--atlas-texto-fraco)]">Limite diário<input type="number" min="10" max="1000" className={`${FIELD_CLASS} cc6-num mt-2`} value={activation.dailyCap} onChange={(e) => setActivation({ ...activation, dailyCap: Number(e.target.value) })} /></label>
              <label className="block text-xs text-[var(--atlas-texto-fraco)]">Intervalo (segundos)<input type="number" min="10" max="3600" className={`${FIELD_CLASS} cc6-num mt-2`} value={activation.intervalSeconds} onChange={(e) => setActivation({ ...activation, intervalSeconds: Number(e.target.value) })} /></label>
            </div>
            <p className="cc6-panel-quiet p-4 text-xs leading-5 text-[var(--atlas-texto-medio)]"><strong className="font-semibold text-[var(--atlas-texto-forte)]">Fluxo protegido:</strong> aprovação única, envio gradual a partir das 9h, limite diário, opt-out em tempo real e pausa por qualidade.</p>
      ...
      <section className="cc6-panel cc6-reveal p-4 sm:p-5" style={{ animationDelay: "360ms" }} aria-labelledby="batches-title">
        <header className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <h2 id="batches-title" className="mt-1 text-lg font-semibold tracking-tight text-[var(--atlas-texto-forte)]">Bases e campanhas</h2>
            <p className="mt-1 text-xs leading-5 text-[var(--atlas-texto-fraco)]">Importação, bloqueios, envios e respostas por base.</p>
          </div>
          ...
          {loading ? (
            ...
            data.batches.map((batch) => (
              <article key={batch.id} className="grid gap-3 border-t border-[rgba(148,163,184,0.12)] py-4 first:border-t-0 md:grid-cols-[1.3fr_.8fr_1fr_auto] md:items-center">
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[var(--atlas-texto-forte)]">{batch.name}</p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {["queued", "running"].includes(batch.status) && batch.quality_status !== "red" ? <button disabled={working} onClick={() => void control(batch.id, "pause")} className={`rounded-md text-xs font-semibold text-[var(--atlas-estado-perigo)] transition-colors hover:text-[var(--atlas-texto-forte)] disabled:opacity-40 ${focusRing}`}>Pausar</button> : null}
                  {batch.quality_status === "red" && data.viewer.role !== "broker" ? <button disabled={working} onClick={() => void control(batch.id, "resume")} className={`rounded-md text-xs font-semibold text-[var(--atlas-estado-sucesso)] transition-colors hover:text-[var(--atlas-texto-forte)] disabled:opacity-40 ${focusRing}`}>Retomar</button> : null}
                </div>
              </article>
            ))
          )}
