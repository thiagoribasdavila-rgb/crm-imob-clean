...
export default function ImportLeadsPage() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="rounded-[24px] border border-emerald-400/15 bg-gradient-to-r from-emerald-400/[.07] to-cyan-400/[.04] p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="atlas-eyebrow">Fase 36 · Proteção comprovada</p><h2 className="mt-2 text-xl font-semibold text-white">Consentimento, duplicidade, opt-out e telefone inválido</h2><p className="mt-2 text-sm text-slate-400">O Atlas verifica na importação e novamente antes da aprovação. Telefones ruins permanecem no histórico de limpeza e não entram novamente. Nenhuma lead existente é transferida silenciosamente.</p></div><div className="flex flex-wrap gap-3"><AtlasBadge tone="danger">{protectionTotals.optOut} OPT-OUTS</AtlasBadge><AtlasBadge tone="warning">{protectionTotals.duplicates} DUPLICADOS</AtlasBadge><AtlasBadge tone="danger">{protectionTotals.invalid} INVÁLIDOS</AtlasBadge></div></div></section>
    ...
    <AtlasCard><AtlasCardHeader eyebrow="Oferta ativa · Corretor + IA" title="Fila segura das bases antigas" description="A IA prioriza e sugere a próxima abordagem; o corretor decide e executa. Telefones inválidos saem da fila e não voltam em novas importações." /><div className="p-5 sm:p-6">{!data?.offers?.length ? <AtlasEmpty title="Nenhuma oferta ativa elegível" description="Importe uma base autorizada ou aguarde novas respostas." /> : <div className="space-y-3">{data.offers.slice(0, 30).map((offer) => { const evidence = phoneEvidence[offer.lead.id] || ""; return <article key={offer.contactId} className="grid gap-4 rounded-2xl border border-white/[.07] bg-white/[.025] p-4 lg:grid-cols-[1fr_1fr_auto] lg:items-center"><div><div className="flex flex-wrap gap-2"><AtlasBadge tone={offer.aiPriority === "high" ? "success" : offer.aiPriority === "medium" ? "info" : "neutral"}>{offer.aiPriority === "high" ? "PRIORIDADE ALTA" : offer.aiPriority === "medium" ? "PRIORIDADE MÉDIA" : "EM APRENDIZADO"}</AtlasBadge><AtlasBadge tone="violet">Score {offer.lead.score ?? 0}</AtlasBadge></div><a href={`/leads/${offer.lead.id}`} className="mt-3 block font-semibold text-white hover:text-cyan-200">{offer.lead.name}</a><p className="mt-1 text-xs text-slate-500">{offer.aiSuggestion}</p></div><input className={inputClass} value={evidence} onChange={(event) => setPhoneEvidence((current) => ({ ...current, [offer.lead.id]: event.target.value }))} placeholder="Ex.: número inexistente confirmado na ligação" maxLength={500} /><button disabled={working || evidence.trim().length < 5} onClick={() => void markInvalidPhone(offer.lead.id)} className="rounded-xl border border-rose-400/20 px-4 py-3 text-xs font-bold text-rose-200 disabled:opacity-40">Telefone inválido</button></article>; })}</div>}</div></AtlasCard>
