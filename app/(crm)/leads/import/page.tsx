...
export default function ImportLeadsPage() {
  ...
  async function decideExperience(signalId: string, experienceDecision: "keep" | "change_requested") {
    setWorking(true); setError("");
    const response = await fetch("/api/v1/crm/reactivation", { method: "POST", headers: { Authorization: `Bearer ${await token()}`, "Content-Type": "application/json" }, body: JSON.stringify({ action: "experience_decision", signalId, experienceDecision }) });
    const result = await response.json();
  ...
  return <div className="space-y-6 pb-10">
    ...
    {data?.experiences.length ? <AtlasCard><AtlasCardHeader eyebrow="IA de experiência" title="Atendimentos que pedem decisão" description="A IA explica o atrito e recomenda recuperação ou oferta de troca. Nenhuma troca é automática." /><div className="space-y-3 p-5 sm:p-6">{data.experiences.map((item) => <article key={item.id} className="rounded-2xl border border-amber-400/15 bg-amber-400/[.045] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex gap-2"><AtlasBadge tone={item.severity === "critical" ? "danger" : "warning"}>{item.severity}</AtlasBadge><AtlasBadge tone="info">{item.confidence}% confiança</AtlasBadge></div><p className="mt-3 font-semibold text-white">{item.evidence}</p><p className="mt-2 text-sm text-slate-400">Sugestão ao cliente: “{item.suggested_reply}”</p><p className="mt-2 text-xs text-slate-500">Corretor atual: {targetMap.get(item.broker_id) || "Responsável atual"}</p></div><div className="flex gap-2"><button disabled={working} onClick={() => void decideExperience(item.id, "keep")} className="atlas-button-secondary">Manter e recuperar</button><button disabled={working} onClick={() => void decideExperience(item.id, "change_requested")} className="atlas-button-primary">Oferecer troca</button></div></div></article>)}</div></AtlasCard> : null}
