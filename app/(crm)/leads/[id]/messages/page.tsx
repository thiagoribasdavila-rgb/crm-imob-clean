...
function objectiveFromIntent(intent: string | null, action: string | null, stage: string | null) {
  if (intent === "summary") return `criar resumo rápido do lead para decidir a próxima ação na etapa ${stage || "atual"}`;
  if (intent === "objections") return "preparar resposta consultiva para objeções de preço, prazo, financiamento e tomada de decisão";
  if (intent === "follow_up") return action || "retomar contato e conduzir a lead para a próxima etapa comercial";
  return "retomar contato e entender o momento de compra";
}

function toneFromIntent(intent: string | null) {
  if (intent === "summary") return "executivo";
  if (intent === "objections") return "consultivo";
  return "acolhedor";
}
...
export default function LeadMessages() {
  ...
  const pipelineContext = {
    fromPipeline: searchParams.get("from") === "pipeline",
  ...
  const [objective, setObjective] = useState(() => objectiveFromIntent(intent, pipelineContext.action, pipelineContext.stage));
  const [tone, setTone] = useState(() => toneFromIntent(intent));
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {pipelineContext.fromPipeline ? <div className="atlas-copilot-context-banner" data-copilot-context="pipeline">
        <span>Contexto recebido do Kanban</span>
        <strong>{pipelineContext.action || "Preparar próxima melhor ação"}</strong>
        <small>{[pipelineContext.project, pipelineContext.campaign, pipelineContext.temperature ? `temperatura ${pipelineContext.temperature}` : null, pipelineContext.score ? `score ${pipelineContext.score}` : null].filter(Boolean).join(" · ") || "A IA vai usar o histórico permitido da lead."}</small>
      </div> : null}
