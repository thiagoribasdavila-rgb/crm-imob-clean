...
export function CommandCenterOverview() {
  ...
  return <section className="space-y-5">
    ...
    <AtlasCard>
      ...
      <div className="grid gap-5 border-t border-white/[.06] p-5 lg:grid-cols-2 sm:p-6">
        <article className="rounded-3xl border border-violet-400/15 bg-violet-400/[.045] p-5">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-white">{calibration.scoreCoverage}%</strong><span className="mt-1 block text-micro text-slate-500">SCORE COBERTO</span></div><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-white">{calibration.contactCoverage}%</strong><span className="mt-1 block text-micro text-slate-500">CONTATO ÚTIL</span></div><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-white">{calibration.outcomeMaturity}%</strong><span className="mt-1 block text-micro text-slate-500">AMOSTRA FINAL</span></div></div>
        ...
        <article className="rounded-3xl border border-cyan-400/15 bg-cyan-400/[.04] p-5">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center"><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-emerald-200">{memory.tiers.focus.toLocaleString("pt-BR")}</strong><span className="mt-1 block text-micro text-slate-500">FOCUS</span></div><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-sky-200">{memory.tiers.watch.toLocaleString("pt-BR")}</strong><span className="mt-1 block text-micro text-slate-500">WATCH</span></div><div className="rounded-xl border border-white/[.06] p-3"><strong className="text-lg text-slate-300">{memory.tiers.suppress.toLocaleString("pt-BR")}</strong><span className="mt-1 block text-micro text-slate-500">SUPPRESS</span></div></div>
    ...
    <div className="text-right text-rotulo text-slate-600">Atualizado em {new Date(data.generatedAt).toLocaleString("pt-BR")} · {data.health.hosting} · {data.health.environment}</div>
  </section>;
}
