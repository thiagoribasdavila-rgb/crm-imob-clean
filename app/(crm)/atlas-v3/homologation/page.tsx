...
type Payload = { checks: HomologationCheck[]; results: Result[]; readiness: { overallEvolution: number; technicalEvolution: number; aiCalibration: { percent: number; controls: number; totalControls: number; scenarios: number; totalScenarios: number; next: string; basis: "evidence_coverage"; accuracyClaimed: false; measuredAt: string | null; dimensions: Array<{ name: string; percent: number }> }; phases: EvolutionPhase[]; deployment: Record<string, boolean> }; currentUser: { id: string; commercialRole: HomologationRole } };
...
export default function HomologationPage() {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <div data-phase="50-productivity-closure"><AtlasCard><AtlasCardHeader eyebrow="Fase 50 · Fechamento de produtividade" title="Fases 41–49 reconciliadas" description="Engenharia aprovada não substitui o teste com usuários e infraestrutura reais." /><div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6 xl:grid-cols-[.7fr_1.3fr]"><div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-5"><AtlasBadge tone="success">ENGENHARIA LOCAL · 100%</AtlasBadge><p className="mt-4 text-sm leading-6 text-slate-300">9 contratos, 9 gates e limites de desempenho auditados para tarefas, alertas, agenda e revisões.</p></div><div className="rounded-2xl border border-amber-400/15 bg-amber-400/[.05] p-5"><AtlasBadge tone="warning">EVIDÊNCIA REAL PENDENTE</AtlasBadge><div className="mt-4 flex flex-wrap gap-2">{["quatro perfis","dois tenants","Mobile","Fuso e cron","Realtime","OAuth","Acessibilidade","Volume real"].map(item=><span key={item} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-300">{item}</span>)}</div><p className="mt-4 text-xs leading-5 text-slate-500">O percentual operacional sobe somente após evidência registrada no roteiro abaixo.</p></div></div></AtlasCard></div>
    <div data-phase="60-distribution-closure"><AtlasCard><AtlasCardHeader eyebrow="Fase 60 · Fechamento de distribuição" title="Fases 51–59 reconciliadas" description="Fila, transferências, ausência, capacidade, prioridade, reserva e auditoria formam um único fluxo governado." /><div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6 xl:grid-cols-[.7fr_1.3fr]"><div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-5"><AtlasBadge tone="success">ENGENHARIA LOCAL · 100%</AtlasBadge><p className="mt-4 text-sm leading-6 text-slate-300">9 contratos, 9 gates, responsável único, atomicidade, capacidade e privacidade reconciliados.</p><Link href="/distribution" className="atlas-button-secondary mt-4">Abrir central de distribuição</Link></div><div className="rounded-2xl border border-amber-400/15 bg-amber-400/[.05] p-5"><AtlasBadge tone="warning">EVIDÊNCIA REAL PENDENTE</AtlasBadge><div className="mt-4 flex flex-wrap gap-2">{["quatro perfis","dois tenants","Equipe online","Concorrência","Cron de reservas","Capacidade real","Mobile","Volume de 10 mil leads"].map(item=><span key={item} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-300">{item}</span>)}</div><p className="mt-4 text-xs leading-5 text-slate-500">Nenhum percentual operacional é liberado sem aceite registrado nos novos testes de gerente, superintendente e corretor.</p></div></div></AtlasCard></div>
    {data ? <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><AtlasMetric label="Evolução do V3" value={`${data.readiness.overallEvolution}%`} detail="Média ponderada das 12 fases" trend="PROJETO" tone="blue" /><AtlasMetric label="Prontidão técnica" value={`${data.readiness.technicalEvolution}%`} detail="Sem maquiar piloto e produção" trend="BUILD" tone="green" /><AtlasMetric label="Cobertura de evidência IA" value={`${data.readiness.aiCalibration.percent}%`} detail={`${data.readiness.aiCalibration.controls}/${data.readiness.aiCalibration.totalControls} controles · ${data.readiness.aiCalibration.scenarios}/${data.readiness.aiCalibration.totalScenarios} cenários`} trend="IA" tone="violet" /><AtlasMetric label="Homologação real" value={`${progress}%`} detail="Somente testes aprovados por usuários" trend="EVIDÊNCIA" tone="amber" /></section> : null}
