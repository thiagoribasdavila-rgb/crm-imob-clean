    ...
    <section className="relative overflow-hidden rounded-[34px] border border-sky-300/15 bg-[#060b16] p-5 shadow-[0_30px_100px_rgba(0,0,0,.35)] sm:p-8" data-program="atlas-2000-phases">
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          {[["Fases", evolution2000Summary.totalPhases], ["Concluídas", evolution2000Summary.completedPhases], ["Bloqueadas", evolution2000Summary.blockedPhases]].map(([label, value]) => <div key={String(label)} className="min-w-20 rounded-2xl border border-white/[.08] bg-white/[.035] p-3 backdrop-blur-xl"><strong className="block text-xl text-white">{value}</strong><span className="text-micro uppercase tracking-[.16em] text-slate-500">{label}</span></div>)}
        </div>
      </div>
      ...
      <div className="mt-5 flex gap-2 overflow-x-auto pb-1" aria-label="Filtrar ondas por pilar">
        {pillars.map((item) => <button key={item} type="button" onClick={() => setPillar(item)} data-active={pillar === item ? "true" : "false"} className="whitespace-nowrap rounded-full border border-white/[.08] px-3.5 py-2 text-micro font-semibold uppercase tracking-[.12em] text-slate-400 transition hover:text-white data-[active=true]:border-sky-300/35 data-[active=true]:bg-sky-300/10 data-[active=true]:text-sky-200">{item}</button>)}
      </div>
      <div className="mt-6 grid gap-5 xl:grid-cols-[.92fr_1.08fr]">
        ...
        {waves.map((wave) => {
          ...
          return <button type="button" onClick={() => setSelectedWaveId(wave.id)} key={wave.id} data-selected={selectedWave.id === wave.id ? "true" : "false"} className="group rounded-2xl border border-white/[.07] bg-white/[.02] p-4 text-left transition hover:border-sky-300/20 hover:bg-white/[.035] data-[selected=true]:border-sky-300/30 data-[selected=true]:bg-sky-300/[.07]">
            <div className="flex items-center justify-between gap-3"><span className="text-micro font-bold uppercase tracking-[.15em] text-sky-300">Onda {String(wave.id).padStart(2, "0")} · {wave.range}</span><span className="rounded-full border border-white/10 px-2 py-1 text-micro uppercase text-slate-500">{wave.pillar}</span></div>
            ...
            <div className="mt-4 flex items-center justify-between text-micro text-slate-500"><span>{phases.length} fases planejadas</span><span className="text-sky-300">Ver detalhes →</span></div>
          </button>;
        })}
        </div>
        <article className="sticky top-5 self-start rounded-[28px] border border-white/[.08] bg-gradient-to-br from-white/[.055] to-white/[.015] p-5 backdrop-blur-xl sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3"><span className="text-micro font-bold uppercase tracking-[.16em] text-sky-300">Onda {String(selectedWave.id).padStart(2, "0")} · fases {selectedWave.range}</span><span className="rounded-full border border-white/10 px-3 py-1 text-micro uppercase tracking-[.12em] text-slate-400">{selectedWave.pillar}</span></div>
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            {selectedPhases.map((phase) => {
              ...
              const blocked = phase.status === "bloqueada";
              return <div key={phase.id} data-complete={complete ? "true" : "false"} data-blocked={blocked ? "true" : "false"} title={blocked ? phase.evidence?.join(" · ") : undefined} className="flex gap-3 rounded-xl border border-white/[.06] bg-black/10 p-3 data-[blocked=true]:border-amber-300/25 data-[blocked=true]:bg-amber-300/[.055] data-[complete=true]:border-emerald-300/20 data-[complete=true]:bg-emerald-300/[.055]"><span className={`font-mono text-micro ${complete ? "text-emerald-300" : blocked ? "text-amber-300" : "text-sky-300"}`}>{complete ? "✓" : blocked ? "!" : String(phase.id).padStart(3, "0")}</span><span className="text-xs leading-5 text-slate-300">{phase.title.split(" · ")[1]}{blocked ? <span className="mt-1 block text-micro font-semibold uppercase tracking-[.14em] text-amber-300">Aguardando staging</span> : null}</span></div>;
            })}
          </div>
      ...
      <p className="mt-5 text-rotulo text-slate-500">Regra: {evolution2000Summary.executionRule}</p>
    </section>
  );
}
