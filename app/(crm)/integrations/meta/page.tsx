...
type Payload = { sources: Source[]; summary: Record<string, number>; conversionConfig: ConversionConfig; conversionSummary: Record<string, number>; conversionFunnel: Record<string, number>; readiness: { webhookSecret: boolean; graphToken: boolean; conversionsToken: boolean; cronWorker: boolean }; canManage: boolean };
...
export default function MetaIntegration() {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <AtlasMetric label="Retentativas" value={(data?.summary.failed ?? 0) + (data?.conversionSummary.failed ?? 0)} detail="Processamento automático" trend="FILA" tone="rose" />
    </section>
    <section className="grid gap-6 xl:grid-cols-2">
      ...
      <AtlasCard><AtlasCardHeader eyebrow="Saúde da conexão" title="Checklist e aprendizado" description="Acompanhe a conexão e os sinais comerciais produzidos por cada avanço real do funil." /><div className="space-y-2 p-5 text-xs text-slate-400 sm:p-6">{data ? <><div className="mb-4 grid grid-cols-2 gap-2">{[["Lead", "Novo lead"], ["Contact", "Contato"], ["QualifiedLead", "Qualificado"], ["Schedule", "Visita"], ["SubmitApplication", "Proposta"], ["ConvertedLead", "Convertido"]].map(([key, label]) => <div key={key} className="rounded-xl border border-white/[.06] bg-white/[.025] p-3"><span className="block text-slate-500">{label}</span><strong className="mt-1 block text-lg text-white">{data.conversionFunnel[key] ?? 0}</strong></div>)}</div>{Object.entries(data.readiness).map(([key, value]) => <div key={key} className="flex justify-between rounded-xl bg-white/[.03] p-3"><span>{key === "webhookSecret" ? "Assinatura do webhook" : key === "graphToken" ? "Token de captura de leads" : key === "conversionsToken" ? "Token de conversões" : "Worker Hostinger"}</span><AtlasBadge tone={value ? "success" : "warning"}>{value ? "PRONTO" : "PENDENTE"}</AtlasBadge></div>)}</> : <AtlasSkeleton className="h-48" />}</div></AtlasCard>
    </section>
  </div>;
}
