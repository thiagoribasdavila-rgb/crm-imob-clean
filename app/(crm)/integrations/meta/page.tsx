...
export default function MetaIntegration() {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <AtlasCard><AtlasCardHeader eyebrow="Audience intelligence" title="O que o time comercial está ensinando" description="Sinais agregados dos acompanhamentos para orientar público, oferta e criativos. Nenhuma descrição ou dado pessoal aparece aqui." /><div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6 lg:grid-cols-5">{data?.audienceRecommendations.length ? data.audienceRecommendations.map((item, index) => <div key={item.signal} className="rounded-2xl border border-violet-400/10 bg-violet-400/[.04] p-4"><span className="text-[10px] uppercase tracking-wider text-violet-300">Prioridade {index + 1}</span><strong className="mt-2 block capitalize text-white">{item.signal.replaceAll("_", " ")}</strong><p className="mt-1 text-xs text-slate-500">{item.count} acompanhamentos</p></div>) : <div className="sm:col-span-2 lg:col-span-5"><AtlasEmpty title="Aguardando acompanhamentos" description="Os motivos mais frequentes aparecerão quando os corretores registrarem preço, região, financiamento, prazo, produto ou concorrência." /></div>}</div></AtlasCard>
    <AtlasCard><AtlasCardHeader eyebrow="Campaign intelligence" title="Qualidade real por campanha" description="Comparação baseada no que aconteceu no CRM depois do formulário — não apenas no volume de leads." /><div className="overflow-x-auto p-5 sm:p-6">{data?.campaignIntelligence.length ? <table className="w-full min-w-[980px] text-left text-xs"><thead className="text-slate-500"><tr className="border-b border-white/[.07]"><th className="p-3">Campanha</th><th className="p-3">Amostra</th><th className="p-3">Qualidade</th><th className="p-3">Score</th><th className="p-3">Visitas</th><th className="p-3">Propostas</th><th className="p-3">Conversão</th><th className="p-3">Diagnóstico</th></tr></thead><tbody>{data.campaignIntelligence.map((campaign) => <tr key={campaign.campaignId} className="border-b border-white/[.05] align-top"><td className="p-3"><strong className="text-white">{campaign.campaignId === "sem-campanha" ? "Origem não identificada" : campaign.campaignId}</strong><p className="mt-1 text-slate-600">{campaign.total} leads · {campaign.buyersElsewhere} compradores externos</p></td><td className="p-3"><AtlasBadge tone={campaign.sampleStatus === "reliable" ? "success" : campaign.sampleStatus === "learning" ? "warning" : "neutral"}>{campaign.sampleStatus === "reliable" ? "CONFIÁVEL" : campaign.sampleStatus === "learning" ? "APRENDENDO" : "INSUFICIENTE"}</AtlasBadge></td><td className="p-3 font-semibold text-white">{campaign.qualityRate}%</td><td className="p-3 text-slate-300">{campaign.averageScore}</td><td className="p-3 text-slate-300">{campaign.visits}</td><td className="p-3 text-slate-300">{campaign.proposals}</td><td className="p-3 font-semibold text-emerald-300">{campaign.conversionRate}%</td><td className="max-w-sm p-3 leading-5 text-slate-400">{campaign.recommendation}</td></tr>)}</tbody></table> : <AtlasEmpty title="Sem amostra de campanhas" description="Quando os leads reais entrarem com o ID da campanha, o Atlas comparará qualidade, visitas, propostas e conversão." />}</div></AtlasCard>
    <AtlasCard><AtlasCardHeader eyebrow="Estratégia Meta atual" title="Automação ampla, sinais comerciais precisos" description="Modelo recomendado para Advantage+ e otimização por leads de qualidade." /><div className="grid gap-3 p-5 sm:grid-cols-2 sm:p-6 xl:grid-cols-4">{[["1", "Controle só o necessário", "Localização, idade mínima, idioma e exclusões operacionais ficam como limites; preferências comerciais entram como sugestões."], ["2", "Ensine com qualidade", "Qualificação, visita, proposta e conversão valem mais do que volume bruto de formulários."], ["3", "Diversifique criativos", "Preço, região, financiamento, prazo e produto orientam novas mensagens e formatos para a Meta testar."], ["4", "Homologue antes de escalar", "Compare qualidade, taxa por etapa e custo por lead qualificado antes de liberar qualquer automação real."]].map(([step, title, description]) => <div key={step} className="rounded-2xl border border-white/[.07] bg-white/[.025] p-4"><AtlasBadge tone="info">PASSO {step}</AtlasBadge><strong className="mt-3 block text-white">{title}</strong><p className="mt-2 text-xs leading-5 text-slate-400">{description}</p></div>)}</div></AtlasCard>
  </div>;
}
