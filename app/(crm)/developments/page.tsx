    ...
    <div className="space-y-6 pb-10">
      ...
      <AtlasCard>
        ...
        <div className="p-5 sm:p-6">
          {loading ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[1,2,3,4,5,6].map((item) => <AtlasSkeleton key={item} className="h-72 w-full" />)}</div> : filtered.length === 0 ? <AtlasEmpty reason={(data?.developments.length ?? 0) > 0 ? "no-results" : "first-use"} eyebrow={(data?.developments.length ?? 0) > 0 ? "Busca sem correspondência" : "Portfólio ainda vazio"} title="Nenhum empreendimento encontrado" description={(data?.developments.length ?? 0) > 0 ? "Nenhum projeto corresponde aos filtros atuais. Limpe os filtros para recuperar o portfólio." : "Cadastre o primeiro empreendimento para reunir estoque, materiais, leads e VGV."} action={(data?.developments.length ?? 0) > 0 ? <button type="button" className="atlas-button-secondary" onClick={() => setQuery("")}>Limpar filtros</button> : <Link href="/developments/registry" className="atlas-button-primary">Cadastrar empreendimento</Link>} /> : <div className="grid gap-5 md:grid-cols-2 2xl:grid-cols-3">{filtered.map((item) => {
