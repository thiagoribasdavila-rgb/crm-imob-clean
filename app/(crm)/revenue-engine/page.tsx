...
export default function RevenueEnginePage() {
  ...
  return <div className="space-y-6 pb-10" data-product="atlas-ai-revenue-engine">
    ...
    {data ? <>
      ...
      <AtlasCard><AtlasCardHeader eyebrow="Da mídia à receita" title="Funil de aprendizado" description="A Meta aprende com resultados profundos; o Atlas preserva a decisão e o contexto comercial."/><div className="grid gap-3 p-5 sm:grid-cols-3 xl:grid-cols-6">{data.funnel.map((stage, index) => <article key={stage.key} className="rounded-2xl border border-white/[.07] bg-white/[.025] p-4"><span className="text-micro font-bold text-cyan-300">0{index + 1}</span><p className="mt-2 text-xs text-slate-400">{stage.label}</p><strong className="mt-3 block text-2xl text-white">{stage.value}</strong></article>)}</div></AtlasCard>
