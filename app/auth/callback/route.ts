...
function publicOrigin(request: NextRequest) {
  if (process.env.NODE_ENV === "production") return "https://atlasaios.com.br";
...
export async function GET(request: NextRequest) {
  ...
  const recoveryFlow =
    rawType === "recovery" || next === "/redefinir-senha" || next === "/reset-password";
  ...
  if (code) {
    ...
    if (error) {
      logger.warn("auth.callback.code_exchange_failed", {
    ...
    if (recoveryFlow) {
      response.cookies.set("atlas-recovery-intent", crypto.randomUUID(), {
        httpOnly: true,
  ...
  if (tokenHash && rawType && allowedOtpTypes.has(rawType as EmailOtpType)) {
    ...
    if (error) {
      logger.warn("auth.callback.token_verification_failed", {
        requestId: meta.requestId,
        correlationId: meta.correlationId,
        status: error.status,
        errorCode: error.code,
