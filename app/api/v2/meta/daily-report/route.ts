...
export async function POST(request: Request) {
  ...
  for (const organization of organizations ?? []) {
    try {
      ...
      const anonymousEvidence = { periods: Object.fromEntries(Object.entries(periods).map(([key, rows]) => [key, rows.slice(0, 10).map((row, index) => ({ campaign: `campaign_${index + 1}`, total: row.total, qualityRate: row.qualityRate, conversionRate: row.conversionRate, averageResponseMinutes: row.averageResponseMinutes, responseCoverage: row.responseCoverage, sla5Rate: row.sla5Rate, sla15Rate: row.sla15Rate, performanceScore: row.performanceScore, sampleStatus: row.sampleStatus }))])), topSignals };
