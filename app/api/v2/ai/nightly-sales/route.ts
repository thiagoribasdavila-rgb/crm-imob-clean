...
function saoPauloHour() {
  return Number(new Intl.DateTimeFormat("en-US", { timeZone: "America/Sao_Paulo", hour: "2-digit", hourCycle: "h23" }).format(new Date()));
}

export async function POST(request: Request) {
  if (!authorized(request)) return NextResponse.json({ error: "Não autorizado." }, { status: 401 });
  if (saoPauloHour() < 20) return NextResponse.json({ prepared: 0, reason: "A jornada comercial inicia após 20h em São Paulo." });
  ...
  for (const lead of leads ?? []) {
    ...
    const [{ data: existing }, { data: suppression }, { data: development }, { count: materialCount }] = await Promise.all([
    ...
    if (existing || suppression) { blocked += 1; continue; }
  ...
  return NextResponse.json({ prepared, blocked, window: "20:00–23:59 America/Sao_Paulo", requiresApproval: true });
}
