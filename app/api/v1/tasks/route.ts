...
export async function POST(request: NextRequest) {
  ...
  if (cadence && (
    ...
    || endsAt.getTime() <= dueAt.getTime()
    || !maxOccurrences
    || maxOccurrences < 2
    || maxOccurrences > 100
  )) return apiError("TASK_RECURRENCE_INVALID", "Defina término futuro e entre 2 e 100 ocorrências.", identity.meta, { status: 400 });
  ...
  if (cadence) {
    const recurring = await getSupabaseAdmin().rpc("create_recurring_task", {
    ...
    if (recurring.error) return apiError("TASK_RECURRENCE_CREATE_FAILED", "Não foi possível criar a repetição. Confirme se a atualização do banco foi aplicada.", identity.meta, { status: 409 });
    structuredApiLog("info", "task.recurrence_created", request, identity.meta, {
      actorId: identity.access.profile.id,
      leadId,
      assigneeId,
      cadence,
      maxOccurrences,
      recurrenceId: recurring.data?.recurrenceId,
    });
    return apiSuccess({
      recurrence: recurring.data,
      ownerPreservedFromLead: Boolean(leadId),
      compatibility: "canonical+legacy-task-columns",
...
export async function PATCH(request: NextRequest) {
  ...
  if (action === "cancel_recurrence") {
    ...
    const task = await admin
      ...
      .maybeSingle();
    if (task.error || !task.data?.recurrence_id) return apiError(
      "TASK_RECURRENCE_NOT_FOUND",
      "Repetição não encontrada ou atualização do banco ainda não aplicada.",
      identity.meta,
      { status: 404 },
    );
    const cancelled = await admin
      ...
      .update({ active: false })
      .eq("id", task.data.recurrence_id)
      .eq("organization_id", identity.access.organization.id)
      .eq("active", true)
      .select("id,active")
      .maybeSingle();
    if (cancelled.error) return apiError("TASK_RECURRENCE_CANCEL_FAILED", "Não foi possível encerrar a repetição.", identity.meta, { status: 409 });
    structuredApiLog("info", "task.recurrence_cancelled", request, identity.meta, {
      actorId: identity.access.profile.id,
      taskId: id,
      recurrenceId: task.data.recurrence_id,
      changed: Boolean(cancelled.data),
    });
    return apiSuccess({
      recurrenceId: task.data.recurrence_id,
      active: false,
