...
const allowedQualityFilters = new Set([
  "missing_project",
  "missing_campaign",
  "missing_source",
]);
...
function createdAfterFilter(raw: string | null) {
...
function createdBeforeFilter(raw: string | null) {
  const value = raw?.trim();
  if (!value) return null;
  const timestamp = Date.parse(value);
  if (!Number.isFinite(timestamp)) return null;
...
function leadIdFilters(raw: string | null) {
  if (!raw) return [];
  return [...new Set(
    raw
      .split(",")
...
export async function GET(request: NextRequest) {
  ...
  const createdAfter = createdAfterFilter(params.get("created_after"));
  const createdBefore = createdBeforeFilter(params.get("created_before"));
  ...
  const leadIds = leadIdFilters(params.get("ids"));
  ...
  const attention = allowedAttentionFilters.has(params.get("attention") ?? "")
    ...
    : null;
  const quality = allowedQualityFilters.has(params.get("quality") ?? "")
    ? params.get("quality")
    : null;
  ...
  if (params.has("ids") && !leadIds.length) {
    return apiError("INVALID_LEAD_IDS", "Selecione leads válidas para continuar.", access.meta, {
      status: 400,
      headers: rate.headers,
    });
  }
  if (params.has("created_after") && !createdAfter) {
    return apiError("INVALID_CREATED_AFTER", "Data inicial do recorte inválida.", access.meta, {
      status: 400,
      headers: rate.headers,
    });
  }
  if (params.has("created_before") && !createdBefore) {
    return apiError("INVALID_CREATED_BEFORE", "Data final do recorte inválida.", access.meta, {
      status: 400,
      headers: rate.headers,
    });
  }
  if (
    ...
    return apiError(
      ...
      "A data final do recorte deve ser posterior à data inicial.",
      access.meta,
  ...
  // Quality filters are read-only review queues. They never change lead data.
  if (quality === "missing_project") query = query.is("project_id", null);
  if (quality === "missing_campaign") query = query.is("campaign_id", null);
  if (quality === "missing_source") query = query.or("source.is.null,source.eq.");
  if (attention) {
