...
export async function GET(request: NextRequest) {
  ...
  const [profileResult, leadResult, campaignResult, projectResult, aiUsageResult] = await Promise.all([
    ...
    admin.from("ai_usage_events").select("total_tokens,latency_ms,estimated_cost_usd,created_at")
  ...
  const forecastItems: CompatRow[] = activeLeads.map((item) => ({
    ...item,
    value: money(item.value) || money(item.budget_max),
  }));
  const forecastWeighted = forecastItems.reduce(
    (sum, item) => sum + money(item.value) * (STAGE_PROBABILITY[normalize(item.status)] ?? 10) / 100,
    0,
  );
  const wonValue = wonLeads.reduce((sum, lead) => sum + money(lead.budget_max), 0);

  const campaignMetrics: CompatRow[] = campaigns.map((campaign) => {
    ...
    return { ...campaign, leads_count: campaignLeads.length, sales_count: sales };
  });
  const campaignRanking = campaignMetrics.map((item) => ({
    id: text(item.id),
    name: text(item.name) || "Campanha",
    channel: text(item.platform) || "Não informado",
    status: text(item.status) || "unknown",
    spend: 0,
    leads: money(item.leads_count),
    sales: money(item.sales_count),
    revenue: 0,
    costPerLead: null,
    conversionRate: money(item.leads_count)
      ? Math.round(money(item.sales_count) / money(item.leads_count) * 1000) / 10
      : 0,
    sampleSufficient: money(item.leads_count) >= 30,
  ...
  const aiUsage = (aiUsageResult.data ?? []).reduce(
    (total, item) => ({
      calls30d: total.calls30d + 1,
      tokens30d: total.tokens30d + money(item.total_tokens),
      estimatedCostUsd30d: total.estimatedCostUsd30d + money(item.estimated_cost_usd),
      latencyTotal: total.latencyTotal + money(item.latency_ms),
    }),
    { calls30d: 0, tokens30d: 0, estimatedCostUsd30d: 0, latencyTotal: 0 },
  );
  const aiUsageSummary = {
    calls30d: aiUsage.calls30d,
    tokens30d: aiUsage.tokens30d,
    estimatedCostUsd30d: Math.round(aiUsage.estimatedCostUsd30d * 1_000_000) / 1_000_000,
    averageLatencyMs30d: aiUsage.calls30d ? Math.round(aiUsage.latencyTotal / aiUsage.calls30d) : 0,
    measured: !aiUsageResult.error,
  };
  ...
  return apiSuccess({
    ...
    financial: { pipelineGross, forecastWeighted, forecastMethod: "crm_probability_weighted", wonValue, commissionReceivable: 0, commissionOverdue: 0 },
