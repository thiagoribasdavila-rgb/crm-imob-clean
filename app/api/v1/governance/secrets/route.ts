...
export async function GET(request: NextRequest) {
  ...
  return apiSuccess({ policy: { valuesReturned: false, source: "config/environment-variables.json", allowedPublicVariables: inventory.filter((item) => item.scope === "public").map((item) => item.name), repositoryScanCommand: "npm run security:secrets" }, variables, summary: { requiredReady: variables.filter((item) => item.requirement === "required").every((item) => item.configured) && alternativeGroupsReady, alternativeGroupsReady, temporaryConfigured: variables.filter((item) => item.requirement === "temporary" && item.configured).length, serverSecretsExposedAsPublic: invalidPublicNames.length, environment: process.env.ATLAS_ENV || "unknown", hosting: process.env.ATLAS_HOSTING_PROVIDER === "hostinger" ? "hostinger" : "unconfirmed" } }, identity.meta, { headers: { ...rate.headers, "Cache-Control": "no-store" } });
}
