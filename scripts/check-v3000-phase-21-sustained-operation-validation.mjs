...
import { verifySustainedOperationAuthorization } from "./check-v3000-phase-20-sustained-operation.mjs";
...
function assertPercent(value, minimum, label) {
  if (!Number.isFinite(value) || value < minimum || value > 100) {
    throw new Error(`${label} deve ficar entre ${minimum}% e 100%.`);
  }
}
...
function pendingResult(status, authorizationResult) {
  return {
    phase: 21,
    ok: true,
    sustainedOperationValidationStatus: status,
    sustainedOperationValidated: false,
    continuousOperationReviewEligible: false,
    automaticProductionActionAllowed: false,
    artifactVerified: authorizationResult.artifactVerified,
    sha256: authorizationResult.sha256,
    sourceFingerprint: authorizationResult.sourceFingerprint,
...
export function evaluateSustainedOperationValidation({
  authorizationResult,
  sustainedOperationValidationEvidence = null,
  contract,
  phase11Contract,
}) {
  assertEqual(contract.phase, 21, "Fase do contrato");
  ...
  assertEqual(authorizationResult.ok, true, "Gate da Fase 20");
  assertEqual(authorizationResult.artifactVerified, true, "Identidade do artefato");
  assertEqual(authorizationResult.sha256, phase11Contract.candidate.sha256, "SHA-256 da release");
  assertEqual(
    authorizationResult.sourceFingerprint,
    phase11Contract.candidate.sourceFingerprint,
    "Fingerprint da release",
  );

  if (!authorizationResult.sustainedOperationAuthorized) {
    return pendingResult("awaiting-sustained-operation-authorization", authorizationResult);
  }
  if (!sustainedOperationValidationEvidence) {
    return pendingResult("awaiting-sustained-operation-validation", authorizationResult);
  }

  const evidence = sustainedOperationValidationEvidence;
  assertEqual(evidence.phase, 21, "Fase da evidência");
  assertEqual(evidence.status, contract.requiredStatus, "Status da validação");
  assertEqual(evidence.candidateSha256, authorizationResult.sha256, "SHA-256 da validação");
  assertEqual(
  ...
  assertEqual(
    ...
    authorizationResult.releaseIdentifier,
  ...
  assertNonNegativeInteger(evidence.observedUserCount, "Usuários observados");
  assertNonNegativeInteger(evidence.userAccessValidatedCount, "Acessos validados");
  assertEqual(
    evidence.observedUserCount,
    authorizationResult.authorizedUserCount,
    "Coorte observada",
  );
  assertEqual(
    evidence.userAccessValidatedCount,
    evidence.observedUserCount,
    "Cobertura de acessos da coorte",
  );

  const roles = normalizedUniqueList(evidence.rolesObserved, "rolesObserved");
  if (roles.length !== contract.requiredRoles.length) {
    throw new Error("A validação deve preservar somente os papéis autorizados.");
  }
  for (const role of contract.requiredRoles) {
    if (!roles.includes(role) || !authorizationResult.rolesAuthorized.includes(role)) {
      throw new Error(`Papel obrigatório não observado: ${role}.`);
    }
  }

  assertNonNegativeInteger(evidence.successfulRequiredJourneys, "Jornadas obrigatórias concluídas");
  assertNonNegativeInteger(evidence.failedRequiredJourneys, "Jornadas obrigatórias com falha");
  if (evidence.successfulRequiredJourneys < contract.minimumSuccessfulRequiredJourneys) {
    throw new Error(
      `São necessárias ao menos ${contract.minimumSuccessfulRequiredJourneys} jornadas obrigatórias concluídas.`,
    );
  }
  assertEqual(
    evidence.failedRequiredJourneys,
    contract.maximumRequiredJourneyFailures,
    "Falhas em jornadas obrigatórias",
  );
  assertPercent(evidence.availabilityPercent, contract.minimumAvailabilityPercent, "Disponibilidade");
  assertPercent(
    evidence.monitoringCoveragePercent,
    contract.minimumMonitoringCoveragePercent,
    "Cobertura de monitoramento",
  );

  assertNonNegativeInteger(evidence.dailyReviewsCompleted, "Revisões diárias concluídas");
  const minimumDailyReviews = Math.ceil(authorizationResult.authorizationWindowHours / 24);
  if (evidence.dailyReviewsCompleted < minimumDailyReviews) {
    throw new Error(`São necessárias ao menos ${minimumDailyReviews} revisões diárias.`);
  }
  ...
  assertEqual(evidence.criticalIncidents, contract.maximumCriticalIncidents, "Incidentes críticos");
  assertEqual(
    evidence.unresolvedHighSeverityIncidents,
  ...
  assertEqual(evidence.validationOwner, authorizationResult.validationOwner, "Responsável pela validação");
  assertEqual(evidence.rollbackOwner, authorizationResult.rollbackOwner, "Responsável pelo rollback");
  assertEqual(
    evidence.observedWindowStart,
    authorizationResult.operationWindowStart,
    "Início da janela observada",
  );
  assertEqual(
    evidence.observedWindowEnd,
    authorizationResult.operationWindowEnd,
    "Fim da janela observada",
  );
  const observedStart = timestamp(evidence.observedWindowStart, "observedWindowStart");
  const observedEnd = timestamp(evidence.observedWindowEnd, "observedWindowEnd");
  const reviewedAt = timestamp(evidence.reviewedAt, "reviewedAt");
  if (observedEnd <= observedStart) throw new Error("A janela observada deve ter duração positiva.");
  if (reviewedAt < observedEnd) {
    throw new Error("A revisão final não pode ocorrer antes do fim da janela observada.");
  }
  ...
  return {
    ...
    automaticProductionActionAllowed: false,
    observedUserCount: evidence.observedUserCount,
    rolesObserved: roles,
    userAccessValidatedCount: evidence.userAccessValidatedCount,
    ...
    sha256: authorizationResult.sha256,
    sourceFingerprint: authorizationResult.sourceFingerprint,
    releaseIdentifier: authorizationResult.releaseIdentifier,
    validationOwner: evidence.validationOwner,
    rollbackOwner: evidence.rollbackOwner,
    reviewedBy: evidence.reviewedBy,
    sanitizedEvidenceReference: evidence.sanitizedEvidenceReference,
    observedWindowStart: evidence.observedWindowStart,
    observedWindowEnd: evidence.observedWindowEnd,
...
export function verifySustainedOperationValidation(input) {
  const authorizationResult = verifySustainedOperationAuthorization(input);
  return evaluateSustainedOperationValidation({
    ...
    contract: input.phase21Contract,
...
function runCli() {
  ...
  console.log(JSON.stringify(verifySustainedOperationValidation({
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    ...
    console.error(`Fase 21 reprovada: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  }
}
