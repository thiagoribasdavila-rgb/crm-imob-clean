...
const checks = [
  ...
  // Catálogo podado na fonte de propósito (commit e20f8931 "navegação podada na fonte"): 19->18 principais e 6->5 contextuais; /command-center consolida Início+Command Center e os grupos (ai)/(autonomous) foram quarentenados. mobilePrimary segue 4.
  ["Catálogo e RBAC permanecem íntegros", primaryCount === 18 && contextCount === 5 && mobilePrimaryCount === 4 && config.catalogPreservation.routesRemoved === 0 && config.catalogPreservation.permissionsChanged === false && config.safetyPolicy.rbacPreserved === true],
