...
const checks = [
  ...
  [
    ...
    sidebar.includes('data-active={active ? "true" : "false"}')
  ...
  [
    ...
    /\.atlas-app-shell\[data-sidebar-collapsed="true"\]\s+\.atlas-nav-item\s+\.atlas-nav-link\s*\{[^}]*padding-inline:\s*10px/.test(styles),
  ],
  ["Catálogo e RBAC permanecem íntegros", primaryCount === 19 && contextCount === 5 && mobilePrimaryCount === 4 && config.catalogPreservation.routesRemoved === 0 && config.catalogPreservation.permissionsChanged === false && config.safetyPolicy.rbacPreserved === true],
