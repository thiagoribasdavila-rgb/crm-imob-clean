...
const checks = [
  ...
  // Catálogo podado na fonte de propósito (commit e20f8931 "navegação podada na fonte") + poda 2026-07-20/21: 16 principais e 7 contextuais; /command-center consolida Início+Command Center e os grupos (ai)/(autonomous) foram quarentenados. mobilePrimary segue 4.
  ["Catálogo e RBAC permanecem íntegros", primaryCount === 16 && contextCount === 7 && mobilePrimaryCount === 4 && config.catalogPreservation.routesRemoved === 0 && config.catalogPreservation.permissionsChanged === false && config.safetyPolicy.rbacPreserved === true],
