...
const startInspection = inspectControlledProofExecutionStartPolicy(
...
const observationInspection = inspectControlledProofExecutionObservationPolicy(
  observationPolicy,
  observationContext,
);
...
const failed = [
  ...
  authorizationMemoryInspection,
  startInspection,
  startMemoryInspection,
  observationInspection,
  observationMemoryInspection,
]
  .find((inspection) => !inspection.ok);
if (failed) throw new Error(`controlled_proof_execution_observation_context_invalid:${failed.reason}`);

console.log(JSON.stringify({
  schema: "atlas.controlled-proof-execution-observation-readiness.v1",
  phase: 229,
  ...
  trustedProofObserversConfigured: observationPolicy.trustedProofObservers.length,
  recordedExecutionStarts: controlledProofExecutionStartMemory.summary.recordedExecutionStarts,
  recordedExecutionObservations: observationMemory.summary.recordedExecutionObservations,
  observedExecutionStarts: observationMemory.summary.observedExecutionStarts,
  distinctObservations: observationMemory.summary.distinctObservations,
  readiness: "awaiting_valid_recorded_execution_start_and_independent_proof_observer",
  controlledProofExecutionObservationAllowed: observationPolicy.controlledProofExecutionObservationAllowed,
  controlledProofExecutionStarted: observationMemory.summary.controlledProofExecutionStarted,
  controlledProofExecutionObserved: observationMemory.summary.controlledProofExecutionObserved,
