...
const checks = [
  ...
  ["Rotas preservam autenticação, organização, RLS e no-store", taskRoute.includes("requireAccessContext(request)") && taskRoute.includes('const organizationId = identity.access.organization.id') && taskRoute.includes('identity.supabase.from("tasks")') && taskRoute.includes('.eq("organization_id", organizationId)') && queueRoute.includes("requireAccessContext(request)") && queueRoute.includes('"Cache-Control": "no-store"') && !queueRoute.includes("getSupabaseAdmin")],
