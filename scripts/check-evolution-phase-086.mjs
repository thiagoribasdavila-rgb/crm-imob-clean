...
const checks = [
  ...
  ["Tenant, lead visível e RLS continuam preservados", taskRoute.includes("requireAccessContext(request)") && taskRoute.includes('const organizationId = identity.access.organization.id') && taskRoute.includes('identity.supabase.from("tasks")') && taskRoute.includes('.eq("organization_id", organizationId)') && taskRoute.includes('from("leads")')],
