...
const policyPath = "config/controlled-proof-execution-subsequent-continuation-observation-review-authorization-consumption-policy.json";
const memoryPath = "config/controlled-proof-execution-subsequent-continuation-observation-review-authorization-consumption-memory.json";
...
if (process.argv.includes("--generate") || process.argv.includes("--write")) {
  const policy = createControlledProofExecutionSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy(
    consumptionContext,
  );
  const memory = createControlledProofExecutionSubsequentContinuationObservationReviewAuthorizationConsumptionMemory({ policy });
...
const authorizationConsumptionPolicy = readJson(policyPath);
const authorizationConsumptionMemory = readJson(memoryPath);
const inspections = [
  inspectControlledProofExecutionSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy(
...
if (failed) {
  throw new Error(`controlled_proof_execution_subsequent_continuation_observation_review_authorization_consumption_context_invalid:${failed.reason}`);
}

console.log(JSON.stringify({
  schema: "atlas.controlled-proof-execution-subsequent-continuation-observation-review-authorization-consumption-readiness.v1",
  phase: 240,
  compositionId: authorizationConsumptionPolicy.compositionId,
  ...
  trustedReviewAuthorizationConsumersConfigured: authorizationConsumptionPolicy.trustedReviewAuthorizationConsumers.length,
  recordedReviews: observationReviewMemory.summary.recordedReviews,
  recordedReviewAuthorizations: reviewAuthorizationMemory.summary.recordedReviewAuthorizations,
  recordedAuthorizationConsumptions: authorizationConsumptionMemory.summary.recordedConsumptions,
  consumedSingleUseAuthorizations: authorizationConsumptionMemory.summary.consumedSingleUseAuthorizations,
  ...
  authorizationConsumptionAllowed: authorizationConsumptionPolicy.authorizationConsumptionAllowed,
  reviewAuthorizationConsumed: authorizationConsumptionMemory.summary.subsequentContinuationAuthorizationConsumed,
  subsequentContinuationAuthorizationConsumed: authorizationConsumptionMemory.summary.subsequentContinuationAuthorizationConsumed,
