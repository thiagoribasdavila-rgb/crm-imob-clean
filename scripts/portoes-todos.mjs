...
const QUARENTENA = {
  ...
  "vigias-vivos:check":
    "AMBIENTE — pergunta ao BANCO quando cada um dos 14 vigias (13 até 02/08/2026, mais `sombra-do-atendimento`) deixou rastro pela última vez, e compara com a cadência de config/workers-schedule.json. Sem credencial sai 2 e NÃO EXECUTADO, nunca verde. Existe porque em 2026-08-01 a medição no banco de produção achou CINCO sinais de automação em zero absoluto — atlas_agent_runs, meta_executions, integration_health_snapshots, nightly_broker_handoffs e meta_daily_reports nunca rodaram uma vez sequer. Ninguém viu, porque `integration-operational-health:check` estava verde o tempo todo conferindo se ARQUIVOS contêm certas strings. Com os carimbos reais medidos à mão, este portão devolve: 4 vigias vivos de 13, 9 que nunca rodaram. Roda no deploy, e é ele que responde se o crontab foi instalado.",
  "rpc-escalacao:check":
