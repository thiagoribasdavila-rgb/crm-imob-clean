...
const checks = [
  ...
  ["Linha de base registra 141 rotas", baseline.crmRoutes === 141 && baseline.canonicalDestinationsPresent === 25],
  ...
  ["Referências internas foram medidas", baseline.navigationReferences === 183 && baseline.uniqueInternalTargets === 68 && baseline.dynamicTemplateReferences === 84],
  ["Catálogo governado cobre todos os destinos", baseline.canonicalRoutesCoveredByGovernedCatalog === 25 && current.structural.canonicalRoutesCoveredByGovernedCatalog === current.structural.canonicalDestinations],
  ["Dependência exclusiva do catálogo está explícita", baseline.canonicalRoutesWithoutExplicitSourceInbound === 9 && config.findings.some((finding) => finding.id === "catalog-only-destinations")],
  ...
  ["Lacuna de entrada de nova lead foi registrada", config.criticalJourneyBaseline.journeysWithoutContextualDirectReference.includes("capturar-nova-lead")],
