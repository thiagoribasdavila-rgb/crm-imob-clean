...
const lazyGlobalSurfaces = fs.readFileSync("components/atlas/lazy-global-surfaces.tsx", "utf8");
...
const checks = [
  ...
  ["Todas as rotas CRM foram classificadas", inventory.counts.crmRoutes === config.topology.crmRoutes],
  ...
  ["Rotas dinâmicas permanecem contextuais", inventory.counts.dynamicContextRoutes === config.topology.dynamicContextRoutes],
  ["Rotas profundas permanecem de apoio", inventory.counts.deepSupportRoutes === config.topology.deepSupportRoutes],
  ["Superfícies de topo foram classificadas", JSON.stringify(measuredTopLevel) === JSON.stringify(expectedTopLevel)],
  ...
  [
    "Shell persistente continua governado pelo layout",
    layout.includes("<AppShell>")
      && appShell.includes("<LazyGlobalSurfaces identity={identity}")
      && lazyGlobalSurfaces.includes("<AtlasCopilotDock")
      && lazyGlobalSurfaces.includes("<AtlasNotificationCenter"),
  ],
  ["Conteúdo preserva estado entre áreas", !appShell.includes("key={pathname}")],
