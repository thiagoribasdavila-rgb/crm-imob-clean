...
const fail = (message) => { throw new Error(`[phase-251] ${message}`); };
const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));
const paths = {
  phase: "config/evolution-phase-251-controlled-proof-execution-further-subsequent-continuation.json",
  policy: "config/controlled-proof-execution-further-subsequent-continuation-policy.json",
  memory: "config/controlled-proof-execution-further-subsequent-continuation-memory.json",
  ...
  documentation: "docs/EVOLUTION_PHASE_251_CONTROLLED_PROOF_EXECUTION_FURTHER_SUBSEQUENT_CONTINUATION.md",
  library: "lib/release/controlled-proof-execution-further-subsequent-continuation.mjs",
  runner: "scripts/run-controlled-proof-execution-further-subsequent-continuation-phase-251.mjs",
  test: "tests/contracts/controlled-proof-execution-further-subsequent-continuation.test.mjs",
};
for (const path of Object.values(paths)) if (!existsSync(path)) fail(`arquivo ausente: ${path}`);

execFileSync(process.execPath, ["scripts/check-evolution-phase-250.mjs"], { stdio: "pipe" });
...
if (phase.phase !== 251 || phase.status !== "implemented") fail("fase ou status inválido");
if (readiness.phase !== 251 || readiness.followingSubsequentContinuationAllowed !== true) {
...
const memoryInspection = inspectControlledProofExecutionFurtherSubsequentContinuationMemory(memory, { policy });
if (!memoryInspection.ok) fail(`memória canônica inválida: ${memoryInspection.reason}`);
if (policy.trustedFurtherSubsequentContinuationExecutors.length !== 0) fail("executor real foi inventado");
...
const bindings = {
  controlledProofExecutionFollowingSubsequentContinuationObservationReviewAuthorizationConsumptionPolicyHash:
    consumptionPolicy.policyHash,
  controlledProofExecutionFollowingSubsequentContinuationObservationReviewAuthorizationConsumptionMemoryHash:
    consumptionMemory.memoryHash,
  controlledProofExecutionFurtherSubsequentContinuationPolicyHash: policy.policyHash,
  controlledProofExecutionFurtherSubsequentContinuationMemoryHash: memory.memoryHash,
};
for (const [key, value] of Object.entries(bindings)) {
  if (phase.currentState[key] !== value || readiness[key] !== value) fail(`vínculo divergente: ${key}`);
}
for (const key of [
  "trustedFurtherSubsequentContinuationExecutorsConfigured",
  "recordedAuthorizationConsumptions",
  "recordedFurtherSubsequentContinuations",
  "consumedAuthorizationConsumptions",
]) if (phase.currentState[key] !== 0 || readiness[key] !== 0) fail(`${key} inventado`);
for (const key of [
  "subsequentContinuationAuthorizationConsumed", "followingSubsequentContinuationAuthorizationConsumed",
...
for (const key of [
  ...
  "authorizationValidityAtContinuationRequired", "signedFurtherSubsequentContinuationReceiptRequired",
  "appendOnlyFurtherSubsequentContinuationMemoryRequired", "duplicateConsumptionContinuationRejected",
  "atomicMemoryHeadBindingRequired", "singleUseFurtherSubsequentContinuationRequired",
]) if (policy[key] !== true || phase.continuationRules[key] !== true) fail(`${key} desabilitado`);
if (policy.maximumFurtherSubsequentContinuations !== 1 || phase.continuationRules.maximumFurtherSubsequentContinuations !== 1) {
  fail("próxima continuação deixou de ser de uso único");
}
if (policy.followingSubsequentContinuationAllowed !== true) fail("próxima continuação interna não habilitada");
for (const key of [
  "followingSubsequentContinuationObservationAllowed", "publicationExecutionAllowed", "networkAccessAllowed",
...
if (phase.nextPhase?.phase !== 252 || phase.nextPhase?.name !== "Controlled Proof Execution Further Subsequent Continuation Observation") {
  fail("próxima fase divergente");
}
if (readJson("config/evolution-program-3000.json").currentPhase < 251) fail("programa principal não avançou");
const scripts = readJson("package.json").scripts ?? {};
for (const name of ["evolution:phase-251:assess", "evolution:phase-251:check"]) {
  if (!scripts[name]) fail(`script ausente: ${name}`);
}

console.log("[phase-251] PASS — a próxima continuação interna exige consumo válido, assinado e registrado, uso único e executor independente; observação e todos os efeitos externos permanecem bloqueados.");
