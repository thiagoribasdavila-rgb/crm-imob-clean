...
const checks = [
  ...
  ["PageHeader usa o componente compartilhado", pageHeader.includes("<AtlasActionLink") && pageHeader.includes('aria-label="Ação do contexto"') && pageHeader.includes('data-action-count="1"')],
  ...
  ["Alvo de página e adaptação móvel estão definidos", styles.includes("min-height: 44px") && /\.atlas-page-actions\s*>\s*\*\s*\{\s*flex:\s*1\s+1\s+auto;\s*\}/m.test(styles) && config.visualBehavior.pageActionMinimumTargetPx === 44],
