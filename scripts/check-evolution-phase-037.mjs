...
const checks = [
  ...
  ["Fila deriva do Pipeline autorizado já carregado", pipeline.includes('data-priority-source="authorized-loaded-pipeline"') && pipeline.includes("const kanbanLensQueue = useMemo") && pipeline.includes("[effectiveKanbanLens, stages, visibleLeads]") && pipeline.includes("kanbanLensQueue") && config.pipelineContract.prioritySource === "authorized-loaded-pipeline"],
  ["Fila permanece limitada a três prioridades", pipeline.includes("const KANBAN_VISIBLE_PRIORITY_LIMIT = 3") && pipeline.includes(".slice(0, KANBAN_VISIBLE_PRIORITY_LIMIT)") && pipeline.includes("data-priority-limit={KANBAN_VISIBLE_PRIORITY_LIMIT}") && config.pipelineContract.visiblePriorityLimit === 3],
  ...
  ["Lead 360, Copilot e ligação permanecem disponíveis", pipeline.includes("Abrir Lead 360") && pipeline.includes('label: "Copilot"') && pipeline.includes("contact.call")],
