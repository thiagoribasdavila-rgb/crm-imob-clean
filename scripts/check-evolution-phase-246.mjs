...
const fail = (message) => { throw new Error(`[phase-246] ${message}`); };
const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));
const paths = {
  phase: "config/evolution-phase-246-controlled-proof-execution-following-subsequent-continuation.json",
  policy: "config/controlled-proof-execution-following-subsequent-continuation-policy.json",
  memory: "config/controlled-proof-execution-following-subsequent-continuation-memory.json",
  ...
  documentation: "docs/EVOLUTION_PHASE_246_CONTROLLED_PROOF_EXECUTION_FOLLOWING_SUBSEQUENT_CONTINUATION.md",
  library: "lib/release/controlled-proof-execution-following-subsequent-continuation.mjs",
  runner: "scripts/run-controlled-proof-execution-following-subsequent-continuation-phase-246.mjs",
  test: "tests/contracts/controlled-proof-execution-following-subsequent-continuation.test.mjs",
};
for (const path of Object.values(paths)) if (!existsSync(path)) fail(`arquivo ausente: ${path}`);

execFileSync(process.execPath, ["scripts/check-evolution-phase-245.mjs"], { stdio: "pipe" });
...
if (phase.phase !== 246 || phase.status !== "implemented") fail("fase ou status inválido");
if (readiness.phase !== 246 || readiness.nextSubsequentContinuationAllowed !== true) {
...
const memoryInspection = inspectControlledProofExecutionFollowingSubsequentContinuationMemory(memory, { policy });
if (!memoryInspection.ok) fail(`memória canônica inválida: ${memoryInspection.reason}`);
if (policy.trustedFollowingSubsequentContinuationExecutors.length !== 0) fail("executor real foi inventado");
...
const bindings = {
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicyHash:
    consumptionPolicy.policyHash,
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemoryHash:
    consumptionMemory.memoryHash,
  controlledProofExecutionFollowingSubsequentContinuationPolicyHash: policy.policyHash,
  controlledProofExecutionFollowingSubsequentContinuationMemoryHash: memory.memoryHash,
};
for (const [key, value] of Object.entries(bindings)) {
  if (phase.currentState[key] !== value || readiness[key] !== value) fail(`vínculo divergente: ${key}`);
}
for (const key of [
  "trustedFollowingSubsequentContinuationExecutorsConfigured",
  "recordedAuthorizationConsumptions",
  "recordedFollowingSubsequentContinuations",
...
for (const key of [
  ...
  "authorizationValidityAtContinuationRequired", "signedFollowingSubsequentContinuationReceiptRequired",
  "appendOnlyFollowingSubsequentContinuationMemoryRequired", "duplicateConsumptionContinuationRejected",
  "atomicMemoryHeadBindingRequired", "singleUseFollowingSubsequentContinuationRequired",
]) if (policy[key] !== true || phase.continuationRules[key] !== true) fail(`${key} desabilitado`);
if (policy.maximumFollowingSubsequentContinuations !== 1 || phase.continuationRules.maximumFollowingSubsequentContinuations !== 1) {
...
if (phase.nextPhase?.phase !== 247 || phase.nextPhase?.name !== "Controlled Proof Execution Following Subsequent Continuation Observation") {
  fail("próxima fase divergente");
}
if (readJson("config/evolution-program-3000.json").currentPhase < 246) fail("programa principal não avançou");
const scripts = readJson("package.json").scripts ?? {};
for (const name of ["evolution:phase-246:assess", "evolution:phase-246:check"]) {
  if (!scripts[name]) fail(`script ausente: ${name}`);
}

console.log("[phase-246] PASS — a próxima continuação interna exige consumo válido, assinado e registrado, uso único e executor independente; observação e todos os efeitos externos permanecem bloqueados.");
