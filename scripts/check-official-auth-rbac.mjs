...
for (const name of ["Thiago Ribas D'Avila", "Senna", "Diego", "Luciano", "Adolfo"]) if (!reset.includes(`name: "${name}"`)) failures.push(`usuário inicial ausente: ${name}`);
...
if (!login.includes("destinationForSession") || !login.includes('return "/command-center"') || !login.includes("readSessionFailure")) failures.push("login sem dashboard adaptativo ou diagnóstico pós-login");
