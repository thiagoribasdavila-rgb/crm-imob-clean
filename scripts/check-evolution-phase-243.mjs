...
const fail = (message) => { throw new Error(`[phase-243] ${message}`); };
const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));
const paths = {
  phase: "config/evolution-phase-243-controlled-proof-execution-next-subsequent-continuation-observation-review.json",
  policy: "config/controlled-proof-execution-next-subsequent-continuation-observation-review-policy.json",
  memory: "config/controlled-proof-execution-next-subsequent-continuation-observation-review-memory.json",
  observationPolicy: "config/controlled-proof-execution-next-subsequent-continuation-observation-policy.json",
  observationMemory: "config/controlled-proof-execution-next-subsequent-continuation-observation-memory.json",
  documentation: "docs/EVOLUTION_PHASE_243_CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW.md",
  library: "lib/release/controlled-proof-execution-next-subsequent-continuation-observation-review.mjs",
  runner: "scripts/run-controlled-proof-execution-next-subsequent-continuation-observation-review-phase-243.mjs",
  test: "tests/contracts/controlled-proof-execution-next-subsequent-continuation-observation-review.test.mjs",
};
for (const path of Object.values(paths)) if (!existsSync(path)) fail(`arquivo ausente: ${path}`);

execFileSync(process.execPath, ["scripts/check-evolution-phase-242.mjs"], { stdio: "pipe" });
...
if (phase.phase !== 243 || phase.status !== "implemented") fail("fase ou status inválido");
if (readiness.phase !== 243 || readiness.nextSubsequentContinuationObservationReviewAllowed !== true) {
...
const memoryInspection = inspectControlledProofExecutionNextSubsequentContinuationObservationReviewMemory(memory, { policy });
if (!memoryInspection.ok) fail(`memória canônica inválida: ${memoryInspection.reason}`);
if (policy.trustedNextSubsequentContinuationObservationReviewers.length !== 0) fail("revisor real foi inventado");
...
const bindings = {
  controlledProofExecutionNextSubsequentContinuationObservationPolicyHash: observationPolicy.policyHash,
  controlledProofExecutionNextSubsequentContinuationObservationMemoryHash: observationMemory.memoryHash,
  controlledProofExecutionNextSubsequentContinuationObservationReviewPolicyHash: policy.policyHash,
  controlledProofExecutionNextSubsequentContinuationObservationReviewMemoryHash: memory.memoryHash,
};
...
for (const key of [
  ...
  "recordedNextSubsequentContinuationObservationReviews",
  "reviewedNextSubsequentContinuationObservations",
  "acceptedNextSubsequentContinuationObservations",
  "rejectedNextSubsequentContinuationObservations",
...
for (const key of [
  ...
  "exactNextSubsequentContinuationBindingRequired", "exactUpstreamChainBindingRequired",
...
if (policy.maximumNextSubsequentContinuationObservationReviewDelaySeconds !== 900 ||
    phase.reviewRules.maximumNextSubsequentContinuationObservationReviewDelaySeconds !== 900) {
  fail("janela de revisão divergente");
}
if (policy.minimumNextSubsequentContinuationObservationReviewReasonLength !== 12 ||
    phase.reviewRules.minimumNextSubsequentContinuationObservationReviewReasonLength !== 12) {
...
if (phase.nextPhase?.phase !== 244 ||
    phase.nextPhase?.name !== "Controlled Proof Execution Next Subsequent Continuation Observation Review Authorization") {
  fail("próxima fase divergente");
}
if (readJson("config/evolution-program-3000.json").currentPhase < 243) fail("programa principal não avançou");
const scripts = readJson("package.json").scripts ?? {};
for (const name of ["evolution:phase-243:assess", "evolution:phase-243:check"]) {
  if (!scripts[name]) fail(`script ausente: ${name}`);
}

console.log("[phase-243] PASS — somente uma observação válida, assinada e registrada pode receber uma revisão interna independente, assinada e de uso único; autorização e efeitos externos permanecem bloqueados.");
