...
if (!assemblyPolicyInspection.ok) throw new Error(`authorized_release_package_assembly_policy_invalid:${assemblyPolicyInspection.reason}`);
const assemblyMemoryInspection = inspectAuthorizedReleasePackageAssemblyMemory(assemblyMemory, { policy: assemblyPolicy });
if (!assemblyMemoryInspection.ok) throw new Error(`authorized_release_package_assembly_memory_invalid:${assemblyMemoryInspection.reason}`);

console.log(JSON.stringify({
  schema: "atlas.authorized-release-package-assembly-readiness.v1",
  phase: 214,
  ...
  readiness: "awaiting_valid_single_use_package_authorization",
