...
if (!endpoint.includes('.eq("assigned_to", identity.access.profile.id)')) failures.push("carteira não filtra explicitamente o corretor");
if (endpoint.includes("getSupabaseAdmin")) failures.push("dashboard do corretor não deve contornar RLS");
if (!page.includes('data-phase="21-broker-daily"')) failures.push("experiência da fase 21 não está visível");
if (!page.includes("Como a IA priorizou")) failures.push("ranking não está explicado ao corretor");
