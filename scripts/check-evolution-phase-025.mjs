...
const checks = [
  ...
  ["Catálogo principal permanece completo", primaryCount === 19 && config.catalogPreservation.primaryDestinations === primaryCount],
  ...
  [
    "Favoritos deixam o grupo somente fora da busca",
    sidebar.includes("const groupedItems = normalizedQuery")
      && sidebar.includes(": secondaryItems.filter((item) => !favorites.includes(item.href))"),
  ],
  [
    "Busca continua incluindo favoritos",
    config.compactionChanges.favoriteDuplication.searchStillReturnsPinnedItems === true
      && sidebar.includes("? visibleItems")
      && !sidebar.includes("visibleItems.filter((item) => !favorites.includes(item.href))"),
  ],
