...
for (const route of routes) {
  ...
  const readsValidatedInput = contract.validationEvidence.some((token) =>
    compactSource.includes(token.replace(/\s+/g, "")),
  );
  if (mutates && !noBodyMutations.has(route) && !readsValidatedInput)
    failures.push(`${route}: mutação sem leitura validável de entrada`);
}
