...
if (process.argv.includes("--generate")) {
  const policy = createControlledProofExecutionNextSubsequentContinuationObservationPolicy(
    nextObservationPolicyContext,
  );
  const memory = createControlledProofExecutionNextSubsequentContinuationObservationMemory({ policy });
...
console.log(JSON.stringify({
  schema: "atlas.controlled-proof-execution-next-subsequent-continuation-observation-readiness.v1",
  phase: 242,
  compositionId: nextObservationPolicy.compositionId,
  ...
  nextSubsequentContinuationObservationAllowed:
    nextObservationPolicy.nextSubsequentContinuationObservationAllowed,
