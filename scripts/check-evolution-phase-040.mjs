...
const checks = [
  ...
  ["API lê e enriquece apenas pelo contexto RLS da organização", activityApi.includes("requireAccessContext") && activityApi.includes('from("activities")') && activityApi.includes('identity.supabase.from("leads")') && activityApi.includes('identity.supabase.from("profiles")') && activityApi.includes('eq("organization_id", organizationId)') && activityApi.includes('scope: "activity-history-read"')],
  ["API é somente leitura e não devolve metadados brutos", !activityApi.includes(".insert(") && !activityApi.includes(".update(") && !activityApi.includes(".delete(") && !activityApi.includes("metadata") && config.activityContract.rawMetadataReturned === false],
