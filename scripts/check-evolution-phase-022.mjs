...
const checks = [
  ...
  ["Inventário anterior está concluído", phaseTwentyOne.status === "completed" && phaseTwentyOne.topology.canonicalDestinationsPresent === 24],
  ["Todos os destinos canônicos possuem resultado", JSON.stringify(canonicalRoutes) === JSON.stringify(outcomeRoutes) && canonicalRoutes.length === 24 && uniqueOutcomeRoutes],
  ["Dezenove destinos principais estão definidos", primaryOutcomes.length === 19],
  ["Cinco comandos contextuais estão definidos", contextualOutcomes.length === 5],
