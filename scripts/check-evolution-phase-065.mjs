...
const checks = [
  ...
  ["Escopo autenticado e tenant permanecem obrigatórios", route.includes("requireAccessContext(request)") && route.includes('const organizationId = identity.access.organization.id') && route.includes('identity.supabase.from("tasks")') && route.includes('.eq("organization_id", organizationId)')],
