...
const pipelineExcludesArchived =
  pipeline.includes("includeArchived: false") ||
  pipeline.includes('.neq("status", "arquivado")') ||
  pipeline.includes('.not("status", "in",');
const dailyWalletExcludesArchived =
  leads.includes('.neq("status", "arquivado")') ||
  leads.includes('.not("status", "in",');
if (!pipelineExcludesArchived) failures.push("pipeline ainda mistura memória fria");
if (!dailyWalletExcludesArchived) failures.push("carteira diária ainda mistura memória fria");
