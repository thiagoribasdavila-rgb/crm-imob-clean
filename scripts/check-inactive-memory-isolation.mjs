...
const home = read("app/(crm)/command-center/page.tsx");
...
if (!home.includes('Perfil aguardando ativação')) failures.push("perfil inativo não possui diagnóstico próprio");
if (home.includes('.from("profiles").select("*").eq("active", true)')) failures.push("dashboard ainda oculta o perfil inativo antes do diagnóstico");
if (!pipeline.includes("readCompatiblePipeline") || !pipeline.includes("archivedMemoryExcluded: true") || !repositories.includes('.not("status", "in", archivedLeadStatuses)')) failures.push("pipeline ainda mistura memória fria");
if (!leads.includes('.not("status", "in", "(arquivado,ARQUIVADO,archived,ARCHIVED)")')) failures.push("carteira diária ainda mistura memória fria");
