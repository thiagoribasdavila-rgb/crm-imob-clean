...
const checks = [
  ...
  ["Histórico usa lead_events real e preserva atividades legadas", writes.includes('.from("lead_events")') && detailRoute.includes("recordLiveLeadEvent") && detailRoute.includes('.from("activities")') && detailRoute.includes('source: "lead_events"') && detailRoute.includes('source: "activities"')],
  ["Rotas ativas não chamam atlas_events", !createRoute.includes('.from("atlas_events")') && !detailRoute.includes('.from("atlas_events")')],
  ["Lead 360 combina relações homologadas e enriquecimento V30", detailRoute.includes('.from("tasks")') && detailRoute.includes('.from("profiles")') && detailRoute.includes('.from("crm_projects")') && detailRoute.includes('.from("opportunities")') && detailRoute.includes('.from("properties")') && detailRoute.includes('.from("lead_experience_signals")')],
