...
async function fixture({ failFirstGate = false } = {}) {
...
function gateDecisions(value, rejectFailed = false) {
...
function adjudicationArguments(value, overrides = {}) {
...
test("política exige adjudicador independente e mantém todos os efeitos externos desligados", async () => {
  const value = await fixture();
  const inspection = inspectReleaseGateResultAdjudicationPolicy(value.adjudicationPolicy, value);
  assert.equal(inspection.ok, true);
  assert.notEqual(value.adjudicator.keyId, value.executor.keyId);
  assert.notEqual(value.adjudicator.actorId, value.authorizer.actorId);
  for (const key of ["automaticReleaseApproval", "automaticReleaseMemoryUpdate", "automaticPackageGeneration", "automaticDeploy", "automaticReleasePromotion"]) assert.equal(value.adjudicationPolicy[key], false);
});

test("conjunto exato de recibos aprovados gera adjudicação assinada sem aprovar release", async () => {
  const value = await fixture();
  const register = createReleaseGateResultAdjudication(adjudicationArguments(value));
  const inspection = inspectReleaseGateResultAdjudication(register, value);
  assert.equal(inspection.ok, true);
  assert.equal(register.status, "gate_results_accepted");
  assert.equal(register.summary.acceptedGates, register.summary.totalGates);
  assert.equal(register.gateResultsAccepted, true);
...
test("gate falho não pode ser aceito e sua rejeição permanece fail-closed", async () => {
  const value = await fixture({ failFirstGate: true });
  assert.throws(() => createReleaseGateResultAdjudication(adjudicationArguments(value)), /failed_gate_result_cannot_be_accepted/);
  const register = createReleaseGateResultAdjudication(adjudicationArguments(value, { gateDecisions: gateDecisions(value, true) }));
  assert.equal(register.status, "gate_results_rejected");
  assert.equal(register.summary.rejectedGates, 1);
  assert.equal(register.gateResultsAccepted, false);
  assert.equal(register.releaseApproved, false);
});

test("executor ou autorizador não pode se tornar adjudicador e janela expirada é bloqueada", async () => {
  const value = await fixture();
  assert.throws(() => createReleaseGateResultAdjudicationPolicy({ packetContext: value.packetContext, decisionPolicy: value.decisionPolicy, authorizationPolicy: value.authorizationPolicy, executionPolicy: value.executionPolicy, trustedAdjudicators: [{ ...value.adjudicator, keyId: value.executor.keyId }] }), /execution_authority_executor_and_adjudicator_must_be_separate/);
  assert.throws(() => createReleaseGateResultAdjudication(adjudicationArguments(value, { decidedAt: "2026-08-09T11:00:00.000Z" })), /gate_result_adjudication_window_expired/);
});

test("adulteração de decisão, vínculo, assinatura ou hash é detectada", async () => {
  const value = await fixture();
  const register = createReleaseGateResultAdjudication(adjudicationArguments(value));
  const decisionTamper = structuredClone(register);
  decisionTamper.gateDecisions[0].reasonCode = "result-inconclusive";
  assert.equal(inspectReleaseGateResultAdjudication(decisionTamper, value).ok, false);
  assert.equal(inspectReleaseGateResultAdjudication({ ...register, executionRegisterHash: "0".repeat(64) }, value).reason, "gate_result_adjudication_execution_register_mismatch");
  assert.equal(inspectReleaseGateResultAdjudication({ ...register, signature: `${register.signature}x` }, value).reason, "gate_result_adjudication_signature_invalid");
  assert.equal(inspectReleaseGateResultAdjudication({ ...register, registerHash: "0".repeat(64) }, value).reason, "gate_result_adjudication_register_hash_mismatch");
});
