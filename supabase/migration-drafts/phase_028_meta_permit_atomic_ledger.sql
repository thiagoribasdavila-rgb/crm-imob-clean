...
-- RASCUNHO LOCAL. NAO E UMA MIGRATION PROMOVIDA E NAO DEVE SER EXECUTADO.
-- Promova somente com `supabase migration new` depois de auditoria humana e clone isolado.
...
comment on function public.atlas_prepare_meta_permit_reservation_v1(uuid, uuid, text, smallint, integer, integer, timestamptz, text, text, text, text, text, text, text, text, text) is
  'Phase 28 draft: atomically reserves one sanitized Meta permit identity. It never issues, consumes or delivers an event.';
