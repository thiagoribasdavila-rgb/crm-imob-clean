...
declare
  actor_role text; target_role text; requested_count integer; accessible_count integer; batch_id uuid;
...
begin
  ...
  select coalesce(commercial_role, case role when 'admin' then 'director' when 'manager' then 'manager' when 'broker' then 'broker' end) into actor_role from public.profiles where id = p_actor_id and organization_id = p_organization_id and active;
  select coalesce(commercial_role, case role when 'admin' then 'director' when 'manager' then 'manager' when 'broker' then 'broker' end) into target_role from public.profiles where id = p_target_owner_id and organization_id = p_organization_id and active;
  ...
  if actor_role <> 'director' and not exists (with recursive descendants as (select id from public.profiles where id = p_actor_id union all select p.id from public.profiles p join descendants d on p.reports_to = d.id where p.active and p.organization_id = p_organization_id) select 1 from descendants where id = p_target_owner_id) then raise exception 'O destino está fora da hierarquia permitida.'; end if;
  if actor_role = 'manager' and target_role <> 'broker' then raise exception 'Gerentes podem transferir apenas para corretores do próprio time.'; end if;
  perform 1 from public.leads where organization_id = p_organization_id and id = any(p_lead_ids) order by id for update;
  with recursive descendants as (select id from public.profiles where id = p_actor_id union all select p.id from public.profiles p join descendants d on p.reports_to = d.id where p.active and p.organization_id = p_organization_id)
  select count(*) into accessible_count from public.leads l where l.organization_id = p_organization_id and l.id = any(p_lead_ids) and (actor_role = 'director' or l.assigned_to in (select id from descendants) or (l.assigned_to is null and actor_role = 'superintendent'));
  ...
  insert into public.lead_transfer_batches (organization_id, actor_id, target_owner_id, lead_count, reason) values (p_organization_id, p_actor_id, p_target_owner_id, requested_count, nullif(trim(p_reason), '')) returning id into batch_id;
  for lead_row in select id, assigned_to from public.leads where organization_id = p_organization_id and id = any(p_lead_ids) order by id loop
    ...
    insert into public.lead_transfer_items (batch_id, lead_id, previous_owner_id, target_owner_id) values (batch_id, lead_row.id, lead_row.assigned_to, actual_target);
