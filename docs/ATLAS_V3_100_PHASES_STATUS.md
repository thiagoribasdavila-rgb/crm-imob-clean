...
- Hostinger, Node.js 22+ (Node 24 recomendado) e Supabase como limites oficiais.
...
O `.env.example` agora nasce em desenvolvimento e não finge estar na Hostinger. O PM2 continua explicitamente em homologação, Node.js 22+ (Node 24 recomendado) e Hostinger. Produção exige outro projeto Supabase e remoção das credenciais temporárias.
