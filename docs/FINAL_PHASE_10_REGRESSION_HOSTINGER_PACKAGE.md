...
Na Hostinger, usar Node.js 22+ (Node 24 recomendado), executar `npm ci`, `npm run prisma:generate`, aplicar as migrations aprovadas no Supabase de homologação e iniciar com `npm start` ou `pm2 start ecosystem.config.cjs`. As chaves devem ser preenchidas no painel da Hostinger, nunca dentro do ZIP.
