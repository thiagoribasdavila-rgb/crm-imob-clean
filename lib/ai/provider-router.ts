...
export type AIProviderResult = { text: string; provider: "openai" | "perplexity" | "local"; model: string; latencyMs: number; citations: string[]; providerRequestId?: string; usage: { inputTokens: number; outputTokens: number; totalTokens: number } };

async function recordUsage(input: GenerateInput, result: AIProviderResult) {
  try {
    await getSupabaseAdmin().from("ai_usage_events").insert({ organization_id: input.organizationId, user_id: input.userId || null, feature: input.feature, task: input.task, provider: result.provider, model: result.model, provider_request_id: result.providerRequestId || null, input_tokens: result.usage.inputTokens, output_tokens: result.usage.outputTokens, total_tokens: result.usage.totalTokens, latency_ms: result.latencyMs });
  } catch {
...
async function generateOpenAI(input: GenerateInput): Promise<AIProviderResult> {
  ...
  const body = await response.json() as { id?: string; error?: { message?: string }; output?: unknown[]; usage?: { input_tokens?: number; output_tokens?: number; total_tokens?: number } };
  ...
  return { text, provider: "openai", model, latencyMs: Date.now() - startedAt, citations: [], providerRequestId: body.id || response.headers.get("x-request-id") || undefined, usage };
}
