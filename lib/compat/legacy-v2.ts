...
export function mapLegacyLead(row: CompatRow): CompatRow {
  ...
  return {
    ...
    // `project` is a legacy free-text field. The compatible repository
    // enriches `development_name` from crm_projects whenever a project_id is
    // available, so consumers can distinguish project from source.
    project_name: first(row, "project_name", "development_name", "project"),
