...
export const evolutionPhases: EvolutionPhase[] = [
  {
    ...
    weight: 8,
    status: "avançada",
    evidence: ["Node 24 validado", "Doctor aprovado", "Build, lint e typecheck aprovados", "306 controles de IA, CRM, Meta e governança e 12 cenários calibrados"],
  ...
  },
  {
    ...
    weight: 13,
    status: "avançada",
    evidence: ["Leads Intelligence paginado", "Lead 360", "Pipeline persistente", "Visibilidade hierárquica", "Transferência em massa", "Fila online por projeto", "Reativação e vendas externas sob governança"],
  ...
  },
  {
    ...
    weight: 10,
    status: "parcial",
    evidence: ["Pipeline com histórico", "SLA visível", "Distribuição equilibrada", "Reativação oficial por WhatsApp", "Lead com corretor e copiloto únicos", "Feedback do cliente auditável"],
  ...
  },
  {
    ...
    weight: 9,
    status: "avançada",
    evidence: ["Copilot persistente e tenant-safe", "306 controles calibrados", "Score e matching explicáveis", "Roteamento por custo e complexidade", "Fallback local determinístico", "OpenAI + Perplexity", "Aprovação humana"],
  ...
  {
    id: 11,
    name: "Comissões e financeiro comercial",
    shortName: "Comissões",
    progress: 68,
    weight: 8,
    status: "parcial",
    evidence: ["SLA por incorporadora", "Vencimento automático", "Estados pendente, próximo, atrasado, parcial, recebido e divergente", "API de configuração e baixa exclusiva da diretoria", "Histórico financeiro versionado"],
    next: "Aplicar migrações, adicionar upload protegido de comprovante e validar pagamentos reais com o financeiro.",
    href: "/sales",
  },
  {
    id: 12,
    name: "Integrações omnichannel",
    shortName: "Integrações",
    progress: 62,
    weight: 7,
    status: "parcial",
    evidence: ["Meta Lead Ads fechado", "WhatsApp por outbox", "Catálogo Google, YouTube, TikTok e portais", "API genérica sem armazenamento de segredos"],
    next: "Implementar OAuth, teste de conexão e sincronização real por provedor antes de declará-los conectados.",
    href: "/integrations",
  },
];
...
export const technicalEvolution = Math.round(
  evolutionPhases
    .filter((phase) => ![9, 10].includes(phase.id))
    .reduce((total, phase) => total + phase.progress * phase.weight, 0) /
    evolutionPhases.filter((phase) => ![9, 10].includes(phase.id)).reduce((total, phase) => total + phase.weight, 0),
);
