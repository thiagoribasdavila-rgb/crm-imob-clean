...
import {
  CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_SCHEMA,
...
} from "./controlled-proof-execution-next-subsequent-continuation-observation-review-authorization.mjs";

export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_POLICY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-review-authorization-consumption-policy.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_RECEIPT_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-review-authorization-consumption-receipt.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-review-authorization-consumption-memory.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_ENTRY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-review-authorization-consumption-memory-entry.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_SIGNATURE_ALGORITHM = "ed25519";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMER_ROLE =
  "next-subsequent-continuation-observation-review-authorization-consumer";
...
function authorizationPolicyContext(args) {
  const {
    ...
    controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy: _consumptionPolicy,
    controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory: _consumptionMemory,
...
function normalizeConsumer(consumer) {
  ...
  if (consumer.role !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMER_ROLE) {
...
export function createControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy({
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationPolicy: authorizationPolicy,
  trustedReviewAuthorizationConsumers = [],
  ...upstream
}) {
  const authorizationInspection = inspectControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationPolicy(
    authorizationPolicy,
  ...
  const payload = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_POLICY_SCHEMA,
    ...
    requiredReviewAuthorizationSchema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_SCHEMA,
    requiredMemoryEntrySchema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_ENTRY_SCHEMA,
    authorizationConsumerRole: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMER_ROLE,
    signatureAlgorithm: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_SIGNATURE_ALGORITHM,
    ...
    exactNextSubsequentContinuationObservationBindingRequired: true,
...
export function inspectControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy(policy, context) {
  try {
    const recreated = createControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy(context);
...
function memoryPayload({ policy, entries }) {
  return {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_SCHEMA,
...
export function createControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory({ policy, entries = [] }) {
  if (policy?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_POLICY_SCHEMA) {
  ...
  for (const [index, entry] of entries.entries()) {
    if (entry?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_ENTRY_SCHEMA) {
    ...
    for (const field of [
      ...
      "reviewMemoryHash", "nextSubsequentContinuationObservationReceiptHash",
      "nextSubsequentContinuationObservationPolicyHash", "nextSubsequentContinuationObservationMemoryHash",
      "nextSubsequentContinuationReceiptHash", "nextSubsequentContinuationPolicyHash",
      "nextSubsequentContinuationMemoryHash", "packageSha256", "inventoryHash",
    ...
    if (
      ...
      entry.nextSubsequentContinuationAllowed !== false
...
export function inspectControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory(memory, { policy }) {
  try {
    const recreated = createControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory({ policy, entries: memory?.entries });
...
function verifyContext(args) {
  const policyInspection = inspectControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy(
...
function authorizationInspectionContext(args) {
  const {
    controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorization: _authorization,
    controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy: _consumptionPolicy,
    controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory: _consumptionMemory,
    trustedReviewAuthorizationConsumers: _consumers,
...
function inspectEligibleAuthorization(authorization, args) {
  ...
  if (
    ...
    authorization.singleUse !== true || authorization.maximumNextSubsequentContinuations !== 1 ||
    authorization.remainingNextSubsequentContinuations !== 1 || authorization.nextSubsequentContinuationExecuted !== false
...
function signingPayload({ authorization, authorizationMemory, consumptionPolicy, consumptionMemoryHashBefore, consumptionId, consumer, consumedAt, nonce }) {
  if (!authorizationMemory.entries.some((entry) => entry.reviewAuthorizationHash === authorization.reviewAuthorizationHash && entry.remainingNextSubsequentContinuations === 1)) {
    throw new Error("controlled_proof_execution_subsequent_continuation_observation_review_authorization_not_recorded");
  }
  return {
    signingSchema: "atlas.controlled-proof-execution-next-subsequent-continuation-observation-review-authorization-consumption-signing-payload.v1",
    ...
    nextSubsequentContinuationObservationReceiptHash: authorization.nextSubsequentContinuationObservationReceiptHash,
    nextSubsequentContinuationObservationPolicyHash: authorization.nextSubsequentContinuationObservationPolicyHash,
    nextSubsequentContinuationObservationMemoryHash: authorization.nextSubsequentContinuationObservationMemoryHash,
    nextSubsequentContinuationReceiptHash: authorization.nextSubsequentContinuationReceiptHash,
    nextSubsequentContinuationPolicyHash: authorization.nextSubsequentContinuationPolicyHash,
    nextSubsequentContinuationMemoryHash: authorization.nextSubsequentContinuationMemoryHash,
    ...
    nextSubsequentContinuationId: authorization.nextSubsequentContinuationId,
...
export function consumeControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorization({
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorization: authorization,
  ...
  const unsigned = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_RECEIPT_SCHEMA,
    ...payload,
    signatureAlgorithm: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_SIGNATURE_ALGORITHM,
    ...
    nextSubsequentContinuationAuthorizationConsumed: true,
    nextSubsequentContinuationAllowed: false,
    nextSubsequentContinuationExecuted: false,
    publicationExecuted: false,
    externalPublicationExecuted: false,
  ...
  const entryPayload = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_MEMORY_ENTRY_SCHEMA,
...
export function inspectControlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionReceipt(receipt, {
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorization: authorization,
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationMemory: authorizationMemory,
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionPolicy: consumptionPolicy,
  controlledProofExecutionNextSubsequentContinuationObservationReviewAuthorizationConsumptionMemory: consumptionMemory,
  ...
  try {
    if (receipt?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_RECEIPT_SCHEMA) {
    ...
    if (receipt.signatureAlgorithm !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_REVIEW_AUTHORIZATION_CONSUMPTION_SIGNATURE_ALGORITHM || typeof receipt.signature !== "string") {
    ...
    if (
      ...
      receipt.nextSubsequentContinuationAuthorizationConsumed !== true || receipt.nextSubsequentContinuationAllowed !== false
