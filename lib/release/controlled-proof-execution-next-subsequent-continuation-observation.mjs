...
import {
  CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_RECEIPT_SCHEMA,
...
} from "./controlled-proof-execution-next-subsequent-continuation.mjs";

export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_POLICY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-policy.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_RECEIPT_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-receipt.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-memory.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_ENTRY_SCHEMA =
  "atlas.controlled-proof-execution-next-subsequent-continuation-observation-memory-entry.v1";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_SIGNATURE_ALGORITHM = "ed25519";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVER_ROLE =
  "controlled-proof-execution-next-subsequent-continuation-observer";
export const CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_KIND =
  "execution-next-subsequent-continuation-confirmed";
...
function normalizeObserver(observer) {
  ...
  if (observer.role !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVER_ROLE) {
...
function inspectPriorPolicy(policy, context) {
  const inspection = inspectControlledProofExecutionNextSubsequentContinuationPolicy(policy, context);
  if (!inspection.ok) {
    throw new Error(`controlled_proof_execution_next_subsequent_continuation_policy_invalid:${inspection.reason}`);
  }
}

export function createControlledProofExecutionNextSubsequentContinuationObservationPolicy({
  controlledProofExecutionNextSubsequentContinuationPolicy: subsequentPolicy,
  controlledProofExecutionNextSubsequentContinuationPolicyContext: subsequentPolicyContext,
  ...
  if (!Array.isArray(trustedNextSubsequentContinuationObservers)) {
    throw new Error("controlled_proof_execution_next_subsequent_continuation_observers_invalid");
  }
  if (!Number.isInteger(maximumNextSubsequentContinuationObservationDelaySeconds) ||
      maximumNextSubsequentContinuationObservationDelaySeconds < 1 ||
      maximumNextSubsequentContinuationObservationDelaySeconds > 3600) {
    throw new Error("controlled_proof_execution_next_subsequent_continuation_observation_delay_invalid");
  }
  const observers = trustedNextSubsequentContinuationObservers.map(normalizeObserver)
  ...
  const payload = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_POLICY_SCHEMA,
    ...
    requiredSubsequentContinuationReceiptSchema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_RECEIPT_SCHEMA,
    requiredMemoryEntrySchema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_ENTRY_SCHEMA,
    observationKind: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_KIND,
    observerRole: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVER_ROLE,
    signatureAlgorithm: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_SIGNATURE_ALGORITHM,
    trustedNextSubsequentContinuationObservers: observers,
    maximumNextSubsequentContinuationObservationDelaySeconds,
...
export function inspectControlledProofExecutionNextSubsequentContinuationObservationPolicy(policy, context) {
  try {
    const recreated = createControlledProofExecutionNextSubsequentContinuationObservationPolicy(context);
    ...
    return { ok: true, policyHash: recreated.policyHash, trustedObservers: recreated.trustedNextSubsequentContinuationObservers.length };
  } catch (error) {
...
function memoryPayload({ policy, entries }) {
  return {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_SCHEMA,
...
export function createControlledProofExecutionNextSubsequentContinuationObservationMemory({ policy, entries = [] }) {
  if (policy?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_POLICY_SCHEMA) {
  ...
  for (const [index, entry] of entries.entries()) {
    if (entry?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_ENTRY_SCHEMA) {
    ...
    if (
      entry.observationKind !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_KIND ||
...
export function inspectControlledProofExecutionNextSubsequentContinuationObservationMemory(memory, { policy }) {
  try {
    const recreated = createControlledProofExecutionNextSubsequentContinuationObservationMemory({ policy, entries: memory?.entries });
...
function verifyContext(args) {
  inspectPriorPolicy(
    ...
    args.controlledProofExecutionNextSubsequentContinuationPolicyContext,
  );
  const subsequentMemoryInspection = inspectControlledProofExecutionNextSubsequentContinuationMemory(
  ...
  const policyInspection = inspectControlledProofExecutionNextSubsequentContinuationObservationPolicy(
  ...
  const memoryInspection = inspectControlledProofExecutionNextSubsequentContinuationObservationMemory(
...
function priorChainContext(args) {
  const {
    controlledProofExecutionNextSubsequentContinuationObservationPolicy: _observationPolicy,
    controlledProofExecutionNextSubsequentContinuationObservationPolicyContext: _observationPolicyContext,
    controlledProofExecutionNextSubsequentContinuationObservationMemory: _observationMemory,
    trustedNextSubsequentContinuationObservers: _trustedObservers,
    maximumNextSubsequentContinuationObservationDelaySeconds: _maximumDelay,
...
function validatePriorReceipt(args) {
  const receiptContext = {
    ...args.controlledProofExecutionNextSubsequentContinuationReceiptContext,
    controlledProofExecutionNextSubsequentContinuationPolicy: args.controlledProofExecutionNextSubsequentContinuationPolicy,
    controlledProofExecutionNextSubsequentContinuationMemory: args.controlledProofExecutionNextSubsequentContinuationMemory,
  };
  const inspection = inspectControlledProofExecutionNextSubsequentContinuationReceipt(
    args.controlledProofExecutionNextSubsequentContinuationReceipt,
    receiptContext,
  );
  if (!inspection.ok) {
    throw new Error(`controlled_proof_execution_next_subsequent_continuation_receipt_invalid:${inspection.reason}`);
  }
  if (!args.controlledProofExecutionNextSubsequentContinuationMemory.entries.some(
    (entry) => entry.nextSubsequentContinuationReceiptHash === args.controlledProofExecutionNextSubsequentContinuationReceipt.nextSubsequentContinuationReceiptHash,
...
function signingPayload({ receipt, subsequentMemory, observationPolicy, observationMemoryHashBefore, observationId, observer, observedAt, nonce }) {
  return {
    signingSchema: "atlas.controlled-proof-execution-next-subsequent-continuation-observation-signing-payload.v1",
    ...
    nextSubsequentContinuationId: receipt.nextSubsequentContinuationId,
...
export function observeControlledProofExecutionNextSubsequentContinuation(args) {
  verifyContext(args);
  validatePriorReceipt(args);
  const receipt = args.controlledProofExecutionNextSubsequentContinuationReceipt;
  const policy = args.controlledProofExecutionNextSubsequentContinuationObservationPolicy;
  const memory = args.controlledProofExecutionNextSubsequentContinuationObservationMemory;
  if (args.observationKind !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_KIND) {
  ...
  const observer = policy.trustedNextSubsequentContinuationObservers.find((item) => item.keyId === args.observerKeyId);
  if (!observer) throw new Error("controlled_proof_execution_next_subsequent_continuation_observer_untrusted");
  validateObservationWindow(receipt, observer, args.observedAt, policy.maximumNextSubsequentContinuationObservationDelaySeconds);
  ...
  const payload = signingPayload({
    receipt,
    subsequentMemory: args.controlledProofExecutionNextSubsequentContinuationMemory,
  ...
  const receiptWithoutHash = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_RECEIPT_SCHEMA,
    ...payload,
    signature,
  };
  ...
  const entryPayload = {
    schema: CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_MEMORY_ENTRY_SCHEMA,
  ...
  return {
    ...
    controlledProofExecutionNextSubsequentContinuationObservationMemory:
      createControlledProofExecutionNextSubsequentContinuationObservationMemory({ policy, entries: [...memory.entries, entry] }),
  };
}

export function inspectControlledProofExecutionNextSubsequentContinuationObservationReceipt(receipt, args) {
  try {
    verifyContext(args);
    validatePriorReceipt(args);
    if (receipt?.schema !== CONTROLLED_PROOF_EXECUTION_NEXT_SUBSEQUENT_CONTINUATION_OBSERVATION_RECEIPT_SCHEMA) {
    ...
    const observer = args.controlledProofExecutionNextSubsequentContinuationObservationPolicy
      .trustedNextSubsequentContinuationObservers.find((item) => item.keyId === receipt.observerKeyId);
    if (!observer) throw new Error("controlled_proof_execution_next_subsequent_continuation_observer_untrusted");
    const identities = collectPriorIdentities(priorChainContext(args));
    collectPriorIdentities(args.controlledProofExecutionNextSubsequentContinuationReceipt, identities);
    if (identities.has(observer.actorId) || identities.has(observer.keyId)) {
      throw new Error("controlled_proof_execution_next_subsequent_continuation_observer_not_independent");
    }
    validateObservationWindow(
      ...
      observer,
      receipt.observedAt,
      args.controlledProofExecutionNextSubsequentContinuationObservationPolicy.maximumNextSubsequentContinuationObservationDelaySeconds,
    );
    const payload = signingPayload({
      receipt: args.controlledProofExecutionNextSubsequentContinuationReceipt,
      subsequentMemory: args.controlledProofExecutionNextSubsequentContinuationMemory,
      observationPolicy: args.controlledProofExecutionNextSubsequentContinuationObservationPolicy,
